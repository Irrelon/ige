...
export async function getFiles (dir: string, matchPattern = ["**/*.ts", "**/*.tsx"], excludePattern = ["node_modules", "react", ".git"]) {
	...
	const files: string[] = await Promise.all(finalList.map((tmpDir) => {
