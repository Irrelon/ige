	...
	init: function () {
		...
		ige.start(function (success) {
			...
			if (success) {
				...
				path1 = self.pathFinder.generate(self.tileMap1, new IgePoint(0, 0, 0), new IgePoint(2, 0, 0), function (tileData, tileX, tileY) {
					// If the map tile data is set to 1, don't allow a path along it
					return tileData !== 1;
				}, true, true);

				// Generate first path, diagonal disabled
				path2 = self.pathFinder.generate(self.tileMap1, new IgePoint(2, 0, 0), new IgePoint(6, 4, 0), function (tileData, tileX, tileY) {
					// If the map tile data is set to 1, don't allow a path along it
					return tileData !== 1;
				}, true, true);
