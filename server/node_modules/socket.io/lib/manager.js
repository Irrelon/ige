...
Manager.prototype.handleClient = function (data, req) {
  ...
  if (handshaken) {
    ...
    if (!this.connected[data.id]) {
	  ...
	  // Handshake has not occurred yet, check if the client
	  // is connecting to a websocket directly bypassing HTTP
	  // request first as would happen if the connecting client
	  // had no access to a DOM!
	  if (data.id === 'handshake') {
		  console.log('socket handshake!');
		  this.handleSocketHandshake(data, req, transport);
	  } else {
		if (transport.open) {
...
Manager.prototype.handleSocketHandshake = function (data, req, transport) {
	var self = this
