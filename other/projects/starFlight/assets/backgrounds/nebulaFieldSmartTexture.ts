...
export const nebulaFieldSmartTexture: IgeSmartTexture = {
	render: (ctx, entity) => {
		let points = 10,
			nebs: NebulaPositionSpeed[],
			sfPoint,
		...
		if (!entity.data("nebula")) {
			...
			if (entity._initialNebula) {
				...
				for (i = 0; i < points; i++) {
					nebs[i] = [
						Math.floor(Math.random() * maxDim * 1.5) - (maxDim * 1.5) / 2, // x position
						Math.floor(Math.random() * maxDim * 1.5) - (maxDim * 1.5) / 2, // y position
						0.1 + parseFloat((Math.random() * 0.5).toFixed(1)), // speed,
						Math.ceil(Math.random() * 4), // type
		...
		const nebula = entity.data("nebula") as Nebula;
		...
		for (i = 0; i < points; i++) {
			...
			texture = ige.textures.get("neb" + sfPoint[3]) as IgeTexture;
