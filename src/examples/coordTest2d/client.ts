...
import { IgeAmbientLight } from "@/engine/webgl/IgeWebGlLight";
import {
	assertPixelColor,
	worldToScreen,
	displayResults,
...
export class Client extends IgeBaseClass implements IgeCanInit {
	...
	async init () {
		...
		const canvas = renderer.canvasElement() as HTMLCanvasElement;
		...
		const testEntities: { name: string; worldPos: IgePoint3d; color: { r: number; g: number; b: number } }[] = [
		...
		const cameraController = (renderer as any)._cameraController;
		...
		for (const config of testEntities) {
			...
			results.push({
				...assertPixelColor(
					...
					canvasW,
					canvasH,
					Math.round(config.color.r * 255),
					Math.round(config.color.g * 255),
					Math.round(config.color.b * 255),
					60
				),
				name: config.name
			});
