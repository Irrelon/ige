...
(function (global) {
	...
	function detectObjectObserve() {
		...
		// chrome release.
		if (records[0].type == "new" && records[1].type == "updated" && records[2].type == "deleted") {
			PROP_ADD_TYPE = "new";
			PROP_UPDATE_TYPE = "updated";
			PROP_RECONFIGURE_TYPE = "reconfigured";
			PROP_DELETE_TYPE = "deleted";
		} else if (records[0].type != "add" || records[1].type != "update" || records[2].type != "delete") {
			console.error("Unexpected change record names for Object.observe. " + "Using dirty-checking instead");
			return false;
		}
		...
		if (records[0].type != ARRAY_SPLICE_TYPE || records[1].type != ARRAY_SPLICE_TYPE) {
			return false;
		}
	...
	function detectEval() {
		...
		// is caught
		if (global.document && "securityPolicy" in global.document && !global.document.securityPolicy.allowsEval) {
			return false;
		}
	...
	function getPath(pathString) {
		...
		if (typeof pathString !== "string") pathString = String(pathString);

		var path = pathCache[pathString];
		if (path) return path;
		if (!isPathValid(pathString)) return invalidPath;
	...
	Path.prototype = createObject({
		...
		getValueFrom: function (obj, observedSet) {
			for (var i = 0; i < this.length; i++) {
				if (obj == null) return;
				if (observedSet) observedSet.observe(obj);
				obj = obj[this[i]];
			}
			return obj;
		},
	...
	Observer.prototype = {
		...
		close: function () {
			if (this.closed_) return;
			if (this.object_ && typeof this.object_.close === "function") this.object_.close();
	...
	ObjectObserver.prototype = createObject({
		...
		connect_: function () {
			if (hasObserve) Object.observe(this.object_, this.boundInternalCallback_);
		},

		sync_: function (hard) {
			if (!hasObserve) this.oldObject = copyObject(this.object_);
		},
		...
		check_: function (changeRecords) {
			...
			if (hasObserve) {
				...
				oldValues = {};
				diff = diffObjectFromChangeRecords(this.object_, changeRecords, oldValues);
			} else {
			...
			this.reportArgs = [diff.added || {}, diff.removed || {}, diff.changed || {}];
		...
		disconnect_: function () {
			...
			else if (this.object_) Object.unobserve(this.object_, this.boundInternalCallback_);
		}
	});
	...
	ArrayObserver.prototype = createObject({
		...
		connect_: function () {
			if (hasObserve) Array.observe(this.object_, this.boundInternalCallback_);
		},

		sync_: function () {
			if (!hasObserve) this.oldObject = this.object_.slice();
		},
		...
		check_: function (changeRecords) {
			...
			if (hasObserve) {
				...
				splices = calcSplices(this.object_, 0, this.object_.length, this.oldObject, 0, this.oldObject.length);
			}
	...
	ObservedSet.prototype = {
		...
		observe: function (obj) {
			if (!isObject(obj) || obj === objProto || obj === arrayProto) return;
	...
	PathObserver.prototype = createObject({
		...
		connect_: function () {
			if (hasObserve) this.observedSet_ = new ObservedSet(this.boundInternalCallback_);
		},
		...
		check_: function () {
			...
			if (this.observedSet_) this.observedSet_.reset();

			this.value_ = this.path_.getValueFrom(this.object_, this.observedSet_);

			if (this.observedSet_) this.observedSet_.cleanup();

			if (areSameValue(this.value_, this.oldValue_)) return false;

			this.value = this.transformFn_ ? this.transformFn_(this.value_) : this.value_;
		...
		setValue: function (newValue) {
			if (this.setValueFn_) this.setValueFn_(newValue);
			else if (this.path_) this.path_.setValueFrom(this.object_, newValue);
		}
	});
	...
	CompoundPathObserver.prototype = createObject({
		...
		addPath: function (object, path) {
			if (this.started_) throw Error("Cannot add more paths once started.");
		...
		getValues_: function () {
			...
			for (var i = 0; i < this.observed_.length; i = i + 2) {
				...
				if (!areSameValue(value, oldValue)) {
					...
					if (!this.transformFn_) this.changeFlags_[i / 2] = true;
		...
		check_: function () {
			...
			if (this.transformFn_) {
				...
				this.reportArgs = [this.values_, this.oldValues_, this.changeFlags_, this.observed_];
			}
		...
		sync_: function (hard) {
			if (hard) {
				this.getValues_();
				if (this.transformFn_) this.value = this.transformFn_(this.values_);
			}

			if (this.transformFn_) this.oldValue = this.value;
		},
		...
		close: function () {
			if (this.observed_) {
				for (var i = 0; i < this.observed_.length; i = i + 2) {
					var object = this.observed_[i];
					if (object && typeof object.close === "function") object.close();
				}
	...
	PathObserver.defineProperty = function (target, name, object, path) {
		...
		var observer = new PathObserver(object, path, function (newValue, oldValue) {
			if (notify) notify(PROP_UPDATE_TYPE, oldValue);
		});
	...
	function diffObjectFromChangeRecords(object, changeRecords, oldValues) {
		...
		for (var i = 0; i < changeRecords.length; i++) {
			...
			if (record.type == PROP_UPDATE_TYPE) continue;
