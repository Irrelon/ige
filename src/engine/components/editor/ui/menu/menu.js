...
create: function (menuData, callback) {
    var self = this;
    self.closeAll();
    ige.editor.renderTemplate(igeRoot + 'components/editor/ui/menu/templates/menu.html', menuData, function (err, htmlElem) {
        if (!err) {
            self._elem = htmlElem;
            ...
            if (menuData.search) {
                ...
                htmlElem.find('.searchInput').keyup(function () {
                    var list = htmlElem.find('ul.items'), items = list.find('li'), searchTerm = $(this).val();
                    if (searchTerm) {
...
add: function (obj) {
    ...
    ige.editor.renderTemplate(igeRoot + 'components/editor/ui/menu/templates/menuButton.html', obj, function (err, htmlElem) {
        if (!err) {
...
buttonOff: function (id) {
    var self = this, obj = self._menus[id], menuButton = $('.dropMenuContainer #' + id);
...
buttonOn: function (id) {
    ...
    if (obj) {
        ...
        // Display menu
        var position = menuButton.offset(), left = position.left, top = position.top, height = $('body').height();
...
toggle: function (id) {
    ...
    if (id) {
        var self = this, menuButton = $('.dropMenuContainer #' + id);
