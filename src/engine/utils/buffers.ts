export function packArraysByFormat<ArrayType> (format: number[], ...dataArgs: ArrayType[][]): ArrayType[] {
...
interface ShaderLocationRange {
	startIndex: number;
	endIndex: number;
	count: number;
}

export function vertexBufferLayoutByFormat (format: number[]) {
	const shaderLocationRanges: ShaderLocationRange[] = [];

	format.forEach((shaderLocationIndex: number, formatIndex) => {
	 ...
	 * ]
	 */
	const attributes = shaderLocationRanges.reduce((attrArr: GPUVertexAttribute[], {
		startIndex,
		count
	}: ShaderLocationRange, locationIndex) => {
...
export const bufferRangeData = (bytesPerItem: number, ...numberOfItems: number[]) => {
	const ranges: number[][] = [];
	let length = 0;
...
export const getMultipleOf = (targetMultiple: number, currentVal: number) => {
