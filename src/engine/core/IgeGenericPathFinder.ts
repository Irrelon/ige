...
import { IgePathFinderListType } from "@/enums/IgePathFinderListType";
import type { IgeCanId } from "@/types/IgeCanId";
import type { IgeGenericPathFinderNode } from "@/types/IgeGenericPathFinderNode";
import type { IgePathFinderFunctionality } from "@/types/IgePathFinderFunctionality";
 ...
 * your application.
 */
export class IgeGenericPathFinder<DataType extends IgeCanId = any> extends IgeEventingClass implements IgePathFinderFunctionality<DataType> {
	classId = "IgeGenericPathFinder";
	...
	_nodeFromData (data: DataType, link?: IgeGenericPathFinderNode<DataType>): IgeGenericPathFinderNode<DataType> {
		...
		return {
			...
			data,
			link,
			listType: IgePathFinderListType.unset
		};
	}
	...
	generate (
		...
		startNode.listType = IgePathFinderListType.open;
		...
		while (openList.length) {
			...
			if (currentNode._id === endNode._id) {
				...
				const finalPath: DataType[] = this._createFinalPathFromNode(currentNode);
			...
			currentNode.listType = IgePathFinderListType.closed;
			...
			while (neighbourCount--) {
				...
				if (!haveSeenNeighbourNode || haveSeenNeighbourNode.listType !== IgePathFinderListType.closed) {
					// The neighbour is not on the closed list so
					// check if it is already on the open list
					if (haveSeenNeighbourNode && haveSeenNeighbourNode.listType === IgePathFinderListType.open) {
						// The neighbour is already on the open list
						// so check if our new path is a better score
						if ((haveSeenNeighbourNode.cost as number) > (neighbourNode.cost as number)) {
							// Pathing from the current node through this neighbour
							// costs less than any way we've calculated before
							haveSeenNeighbourNode.link = neighbourNode.link as IgeGenericPathFinderNode<DataType>;
							haveSeenNeighbourNode.cost = neighbourNode.cost as number;
						}
					} else {
						// Add the neighbour to the open list
						const processedNeighbourNode = neighbourNode;
						neighbourNode.listType = IgePathFinderListType.open;
				...
				if (!lowestCostNode || (haveSeenNeighbourNode.cost as number) < (lowestCostNode.cost as number)) {
		...
		const finalPath: DataType[] = this._createFinalPathFromNode(lowestCostNode);
	...
	_getLowestCostNodeIndex (nodeArr: IgeGenericPathFinderNode<DataType>[]): number {
		...
		while (openCount--) {
			if ((nodeArr[openCount].cost as number) < (nodeArr[lowestCostIndex].cost as number)) {
