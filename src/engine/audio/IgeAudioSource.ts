...
export class IgeAudioSource extends IgeAsset {
	...
	constructor (id?: string, urlOrObject?: string | IgeSmartAudioSource) {
		...
		if (isServer) {
			this.log(
				`Cannot create an audio source on the server. Audio sources are only client-side objects. Please alter your code so that you don't try to load an audio source on the server-side, using something like an if statement around your code such as "if (isClient) {...}".`,
				"error"
			);
			return this;
		}
	...
	url (url: string): this;
	url (url?: string): string | this | undefined {
		if (url === undefined) {
			return this._url;
		}
	 ...
	 * @private
	 */
	assignSmartAudioSource (scriptObj: IgeSmartAudioSource) {
