...
export const hashToExtents = (hash: string): vec4 => {
	let extents: vec4 = [-1, -1, 1, 1];
...
export const extentsToHash = ([x1, y1, x2, y2]: [number, number, number, number], level: number = 1): string => {
	let hash = "";

	const currentQuad: vec4 = [-1, -1, 1, 1];
