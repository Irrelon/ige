...
describe("Coordinate System", () => {
	describe("lookAt basis vectors (right-handed)", () => {
		it("produces correct basis vectors for camera on +Z axis", () => {
			...
			m.lookAt(
				new IgePoint3d(0, 0, 5),
				new IgePoint3d(0, 0, 0),
				new IgePoint3d(0, 1, 0)
			);
	...
	describe("NDC to screen coordinate mapping", () => {
		 ...
		 */
		function ndcToScreen (ndcX: number, ndcY: number, width: number, height: number) {
			return {
				x: (ndcX + 1) / 2 * width,
				y: (1 - ndcY) / 2 * height
			};
