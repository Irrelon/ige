...
class MyPathFinder extends IgeGenericPathFinder<GraphItem> {
	getGraphItem (id: string) {
		return graph.find((item) => item._id === id);
	}

	getConnections (currentNode: GraphItem, targetNode: GraphItem): GraphItem[] {
		// Get the current graph data
		const graphItem = this.getGraphItem(currentNode._id);
		if (!graphItem) return [];

		return (graphItem.connectsTo.map((connectionId): GraphItem | null => {
			const connectionItem = this.getGraphItem(connectionId);
			if (!connectionItem) return null;

			return connectionItem;
		})).filter((item) => item !== null) as GraphItem[];
	}
}
