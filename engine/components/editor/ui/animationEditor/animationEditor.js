...
setupListeners: function (dialogElem) {
    ...
    self._cellsCanvas.on('click', function (e) {
        var oe = e.originalEvent, cell = self.cellFromXY(oe), frameIndex;
...
cellFromIndex: function (index) {
    index = index - 1;
    var self = this, cellsPerRow = (self._textureImage.width / self._cellWidth), y = Math.floor(index / cellsPerRow), x = index - (y * cellsPerRow);
    return { x: x, y: y };
}
...
_renderCanvas: function (noGrid) {
    var self = this, framesCtx = self._framesCtx, cellsCtx = self._cellsCtx, outputCtx = self._outputCtx, cell, cellWidth, cellHeight, x, y, i, cellIndex;
