...
class IgeSocketIoServerComponent {
	_idCounter: number = 0;
	_requests: Record<string, IgeNetworkRequestMessageData> = {};
     ...
     * network has started.
     */
	start (port?: string, callback?: () => void) {
		this._socketById = {};
     ...
     * @return {*}
     */
	define (commandName?: string, callback?: () => void) {
	...
	_onRequest = (data: IgeNetworkMessageData, clientId: string) => {
     ...
     * @private
     */
	_onClientDisconnect (data: IgeNetworkMessageData, socket) {
