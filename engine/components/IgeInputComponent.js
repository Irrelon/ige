var IgeInputComponent = IgeEventingClass.extend({
	...
	_mouseDown: function (event) {
		...
		var mx = event.igeX - ige.bounds.bounds2d().x2,
			my = event.igeY - ige.bounds.bounds2d().y2,
			self = this;
	...
	_updateMouseData: function (event) {
		...
		var arr = ige._children,
			arrCount = arr.length,
			vp, vpUpdated,
			mx = (event.igeX - ige.bounds.bounds2d().x2) - ige._translate.x,
			my = (event.igeY - ige.bounds.bounds2d().y2) - ige._translate.y;
		...
		while (arrCount--) {
			...
			if (mx > vp._translate.x - vp.bounds.bounds2d().x / 2 && mx < vp._translate.x + vp.bounds.bounds2d().x / 2) {
				if (my > vp._translate.y - vp.bounds.bounds2d().y / 2 && my < vp._translate.y + vp.bounds.bounds2d().y / 2) {
