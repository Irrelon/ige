...
class IgeInputComponent extends IgeEventingClass {
	...
	setupListeners = (canvas) => {
		...
		this._evRef = {
			"mousedown": (event) => { event.igeType = "mouse"; this._rationalise(event); this._mouseDown(event); },
			"mouseup": (event) => { event.igeType = "mouse"; this._rationalise(event); this._mouseUp(event); },
			"mousemove": (event) => { event.igeType = "mouse"; this._rationalise(event); this._mouseMove(event); },
			"mouseWheel": (event) => { event.igeType = "mouse"; this._rationalise(event); this._mouseWheel(event); },

			"touchmove": (event) => { event.igeType = "touch"; this._rationalise(event, true); this._mouseMove(event); },
			"touchstart": (event) => { event.igeType = "touch"; this._rationalise(event, true); this._mouseDown(event); },
			"touchend": (event) => { event.igeType = "touch"; this._rationalise(event, true); this._mouseUp(event); },

			"contextmenu": (event) => { event.preventDefault(); event.igeType = "mouse"; this._rationalise(event); this._contextMenu(event); },

			"keydown": (event) => { event.igeType = "key"; this._rationalise(event); this._keyDown(event); },
			"keyup": (event) => { event.igeType = "key"; this._rationalise(event); this._keyUp(event); }
		};
