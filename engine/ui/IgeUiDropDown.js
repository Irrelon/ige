var IgeUiDropDown = IgeUiElement.extend({
		...
		if (this._toggleState) {
			...
			for (i = 0; i < this._options.length; i++) {
				...
				new IgeUiLabel()
					...
					.top((this._geometry.y * i) + 1)
					.left(1)
					.width(this._geometry.x - 2)
					.height(this._geometry.y - 2)
					.allowFocus(true)
					.allowActive(true)
					.allowHover(true)
					.mouseUp(function () {
	...
	tick: function (ctx) {
		...
		ctx.fillRect(Math.floor(this._geometry.x2) - 30, -this._geometry.y2 + 1, 30, this._geometry.y - 2);
		
		// Chevron
		ctx.strokeStyle = this._color;
		ctx.beginPath();
		ctx.moveTo(this._geometry.x2 - 18.5, -this._geometry.y2 + 14.5);
		ctx.lineTo(this._geometry.x2 - 14.5, 2.5);
		ctx.lineTo(this._geometry.x2 - 10.5, -this._geometry.y2 + 14.5);
		ctx.stroke();
