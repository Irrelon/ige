var IgeUiDropDown = IgeUiElement.extend({
	...
	toggle: function () {
		...
		if (this._toggleState) {
			var self = this,
				optionContainer,
				mainTop = this.bounds.bounds2d().y + 5,
			...
			for (i = 0; i < this._options.length; i++) {
				...
				new IgeUiLabel()
					...
					.top((this.bounds.bounds2d().y * i) + 1)
					.left(1)
					.width(this.bounds.bounds2d().x - 2)
					.height(this.bounds.bounds2d().y - 2)
					.allowFocus(true)
					.allowActive(true)
					.allowHover(true)
					.mouseUp(function () {
	...
	tick: function (ctx) {
		...
		ctx.fillRect(Math.floor(this.bounds.bounds2d().x2) - 30, -this.bounds.bounds2d().y2 + 1, 30, this.bounds.bounds2d().y - 2);
		
		// Chevron
		ctx.strokeStyle = this._color;
		ctx.beginPath();
		ctx.moveTo(this.bounds.bounds2d().x2 - 18.5, -this.bounds.bounds2d().y2 + 14.5);
		ctx.lineTo(this.bounds.bounds2d().x2 - 14.5, 2.5);
		ctx.lineTo(this.bounds.bounds2d().x2 - 10.5, -this.bounds.bounds2d().y2 + 14.5);
		ctx.stroke();
