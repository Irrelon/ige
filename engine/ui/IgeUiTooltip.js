...
var IgeUiTooltip = IgeUiElement.extend({
	...
	setContent: function (val) {
		if (val !== undefined) {
			...
			if (typeof(val) == 'string') {
				...
				this.textBox.height(this.bounds.bounds2d().y);
				this.textBox.top(0);
				...
				this.textBox.mount(this);
				this.textBox.height(this.bounds.bounds2d().y - this.titleBox.bounds.bounds2d().y);
				this.textBox.top(this.titleBox.bounds.bounds2d().y);
				//title + text
	...
	_mousemove: function (event) {
		...
		tt.translateTo(event.igeX - mountPos.x + tt.bounds.bounds2d().x2 + 10, event.igeY - mountPos.y + tt.bounds.bounds2d().y2, 0);
		tt.updateUiChildren();
	},
