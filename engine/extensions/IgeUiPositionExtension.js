var IgeUiPositionExtension = {
			...
			if (px === null) {
				...
				if (typeof(px) === 'string') {
					...
					if (this._parent) {
						...
						parentWidth = this._parent.bounds.bounds2d().x;
					} else {
						// We don't have a parent so use the main canvas
						// as a reference
						parentWidth = ige.bounds.bounds2d().x;
					}
		...
		if (px !== undefined) {
			if (px === null) {
				...
				if (typeof(px) === 'string') {
					...
					if (this._parent) {
						// We have a parent, use it's geometry
						parentWidth = this._parent.bounds.bounds2d().x2;
					} else {
						// We don't have a parent so use the main canvas
						// as a reference
						parentWidth = ige.bounds.bounds2d().x2;
					}
		...
		if (px !== undefined) {
			if (px === null) {
				...
				if (typeof(px) === 'string') {
					...
					if (this._parent) {
						// We have a parent, use it's geometry
						parentHeight = this._parent.bounds.bounds2d().y;
					} else {
						// We don't have a parent so use the main canvas
						// as a reference
						parentHeight = ige.bounds.bounds2d().y;
					}
		...
		if (px !== undefined) {
			if (px === null) {
				...
				if (typeof(px) === 'string') {
					...
					if (this._parent) {
						// We have a parent, use it's geometry
						parentWidth = this._parent.bounds.bounds2d().y2;
					} else {
						// We don't have a parent so use the main canvas
						// as a reference
						parentWidth = ige.bounds.bounds2d().y2;
					}
	...
	width: function (px, lockAspect, modifier, noUpdate) {
		if (px !== undefined) {
			if (px === null) {
				...
				this.bounds.bounds2d().x = 0;
				this.bounds.bounds2d().x2 = 0;
			} else {
				this._uiWidth = px;
				...
				if (typeof(px) === 'string') {
					if (this._parent) {
						// Percentage
						var parentWidth = this._parent.bounds.bounds2d().x,
							val = parseInt(px, 10),
							newVal,
							ratio;
						...
						if (lockAspect) {
							...
							ratio = newVal / this.bounds.bounds2d().x;
							this.height(this.bounds.bounds2d().y / ratio, false, 0, noUpdate);
						}
	
						this.bounds.bounds2d().x = newVal;
						this.bounds.bounds2d().x2 = Math.floor(this.bounds.bounds2d().x / 2);
					} else {
						// We don't have a parent so use the main canvas
						// as a reference
						var parentWidth = ige.bounds.bounds2d().x,
							val = parseInt(px, 10);
	
						// Calculate real height from percentage
						this.bounds.bounds2d().x = (parentWidth / 100 * val) + this._widthModifier | 0;
						this.bounds.bounds2d().x2 = Math.floor(this.bounds.bounds2d().x / 2);
					}
				} else {
					if (lockAspect) {
	...
	height: function (px, lockAspect, modifier, noUpdate) {
		if (px !== undefined) {
			if (px === null) {
				...
				this.bounds.bounds2d().y = 0;
				this.bounds.bounds2d().y2 = 0;
			} else {
				this._uiHeight = px;
				...
				if (typeof(px) === 'string') {
					if (this._parent) {
						// Percentage
						var parentHeight = this._parent.bounds.bounds2d().y,
							val = parseInt(px, 10),
							newVal,
							ratio;
						...
						if (lockAspect) {
							// Calculate the height from the change in width
							ratio = newVal / this.bounds.bounds2d().y;
							this.width(this.bounds.bounds2d().x / ratio, false, 0, noUpdate);
						}
	
						this.bounds.bounds2d().y = newVal;
						this.bounds.bounds2d().y2 = Math.floor(this.bounds.bounds2d().y / 2);
					} else {
						// We don't have a parent so use the main canvas
						// as a reference
						var parentHeight = ige.bounds.bounds2d().y,
							val = parseInt(px, 10);
	
						// Calculate real height from percentage
						this.bounds.bounds2d().y = (parentHeight / 100 * val) + this._heightModifier | 0;
						this.bounds.bounds2d().y2 = Math.floor(this.bounds.bounds2d().y / 2);
					}
				} else {
					if (lockAspect) {
	...
	_updateUiPosition: function () {
		if (this._parent) {
			var parentGeom = this._parent.bounds.bounds2d(),
				geomScaled = this.bounds.bounds2d().multiplyPoint(this._scale),
				percent,
				newVal,
				ratio;
