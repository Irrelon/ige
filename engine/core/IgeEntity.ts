...
class IgeEntity extends IgeObject {
	...
	cache (val?: boolean, propagateToChildren = false) {
		...
		if (this._ige.isClient) {
			// Use a real canvas
			const canvasObj = this._ige.createCanvas({"smoothing": this._cacheSmoothing});
	 ...
	 * @return {*}
	 */
	compositeCache (val, propagateToChildren) {
		if (!this._ige.isClient) {
			return this;
		}
		...
		this._children.forEach((child) => {
			if (val) {
				child._compositeParent = true;
			} else {
				delete child._compositeParent;
			}

			if (propagateToChildren === true) {
				child.compositeCache && child.compositeCache(val, propagateToChildren);
			}
		});
	...
	rotateToPoint (point) {
		...
		this.rotateTo(
			this._rotate.x,
			this._rotate.y,
			(Math.atan2(worldPos.y - point.y, worldPos.x - point.x) - this._parent._rotate.z) + IgeBaseClass.degreesToRadians(270)
		);
	...
	screenPosition () {
		return new IgePoint3d(
			Math.floor(((this._worldMatrix.matrix[2] - this._ige._currentCamera._translate.x) * this._ige._currentCamera._scale.x) + this._ige.root._bounds2d.x2),
			Math.floor(((this._worldMatrix.matrix[5] - this._ige._currentCamera._translate.y) * this._ige._currentCamera._scale.y) + this._ige.root._bounds2d.y2),
			0
		);
	}
	...
	localBounds3dPolygon (recalculate) {
		if (this._bounds3dPolygonDirty || !this._localBounds3dPolygon || recalculate) {
			var geom = this._bounds3d,
				poly = new IgePoly2d(),
				// Bottom face
				bf2 = IgeBaseClass.toIso(+(geom.x2), -(geom.y2),  -(geom.z2)),
				bf3 = IgeBaseClass.toIso(+(geom.x2), +(geom.y2),  -(geom.z2)),
				bf4 = IgeBaseClass.toIso(-(geom.x2), +(geom.y2),  -(geom.z2)),
				// Top face
				tf1 = IgeBaseClass.toIso(-(geom.x2), -(geom.y2),  (geom.z2)),
				tf2 = IgeBaseClass.toIso(+(geom.x2), -(geom.y2),  (geom.z2)),
				tf4 = IgeBaseClass.toIso(-(geom.x2), +(geom.y2),  (geom.z2));
	...
	tick (ctx, dontTransform= false) {
		if (!(!this._hidden && this._inView && (!this._parent || (this._parent._inView)) && !this._streamJustCreated)) { return; }
		...
		if (this._mouseEventsActive) {
			if (this._processTriggerHitTests()) {
				...
				this._ige.input.queueEvent(this._mouseInTrigger, null);
			} else {
	...
	_stringify (options) {
		...
		var str = super._stringify(options), i;
	 ...
	 */
	debugTransforms () {
		this._ige.traceSet(this._translate, "x", 1, (val) => {
			return isNaN(val);
		});

		this._ige.traceSet(this._translate, "y", 1, (val) => {
			return isNaN(val);
		});

		this._ige.traceSet(this._translate, "z", 1, (val) => {
			return isNaN(val);
		});

		this._ige.traceSet(this._rotate, "x", 1, (val) => {
			return isNaN(val);
		});

		this._ige.traceSet(this._rotate, "y", 1, (val) => {
			return isNaN(val);
		});

		this._ige.traceSet(this._rotate, "z", 1, (val) => {
			return isNaN(val);
		});

		this._ige.traceSet(this._scale, "x", 1, (val) => {
			return isNaN(val);
		});

		this._ige.traceSet(this._scale, "y", 1, (val) => {
			return isNaN(val);
		});

		this._ige.traceSet(this._scale, "z", 1, (val) => {
			return isNaN(val);
		});
	 ...
	 *     entity.rotateBy(0, 0, IgeBaseClass.degreesToRadians(10));
	 * @return {*}
	 */
	rotateBy (x, y, z) {
		if (x !== undefined && y!== undefined && z !== undefined) {
	 ...
	 *     entity.rotateTo(0, 0, IgeBaseClass.degreesToRadians(10));
	 * @return {*}
	 */
	rotateTo (x, y, z) {
		if (x !== undefined && y!== undefined && z !== undefined) {
	 ...
	 *     entity.rotate().z(IgeBaseClass.degreesToRadians(10));
	 * @return {*}
	 */
	rotate (...args) {
		if (args.length) {
	...
	streamSectionsPull (sectionName) {
		if (this._streamSections) {
			IgeBaseClass.pull(this._streamSections, sectionName);
		}
