...
export class IgeEngine extends IgeEntity {
	...
	_currentCamera: IgeCamera;
	_globalSmoothing: boolean = false;
	_currentViewport: IgeViewport | null;
	_register: Record<string, IgeObject> = {};
	_groupRegister: Record<string, IgeObject[]> = {};
	_categoryRegister: Record<string, IgeObject> = {};
    _canvas: HTMLCanvasElement;
	_ctx: CanvasRenderingContext2D;
	_textureStore: Record<string, IgeTexture>;
	_postTick: (() => void)[];
    ...
    _showSgTree: boolean;
    _renderContext: string;
    _renderMode: number;
    _pixelRatioScaling: number;
    _autoSize: boolean;
    _state: number;
    _syncIndex: number;
    _syncArr: SyncEntry[];
	...
	constructor (props: IgeClassProps) {
		...
		if (props.this._igeConfig.debug) {
			if (!props.this._igeConfig.debug._enabled) {
				// Debug is not enabled so ensure that
				// timing debugs are disabled
				props.this._igeConfig.debug._timing = false;
			}
		}
		...
		this._renderMode = this._renderModes.indexOf(this._renderContext); // Integer representation of the render context
	 ...
	 * a call to this._ige.$(objectId).
	 * @param {Object} obj The object to register.
	 * @return {*}
	 */
	register (obj: IgeObject) {
		if (obj !== undefined) {
	 ...
	 * object will no longer be accessible via this._ige.$().
	 ...
	 * a call to this._ige.$$(categoryName).
	 * @param {Object} obj The object to register.
	 * @return {*}
	 */
	 ...
	 * @return {*}
	 */
	categoryUnRegister (obj?: IgeObject) {
		if (obj !== undefined) {
	 ...
	 * register allows you to access an object by its groups with
	 * a call to this._ige.$$$(groupName).
	 * @param {Object} obj The object to register.
	 * @param {String} groupName The name of the group to register
	 * the object in.
	 * @return {*}
	 */
	groupRegister (obj?: IgeObject, groupName: string) {
		if (obj !== undefined) {
	 ...
	 * @return {*}
	 */
	groupUnRegister (obj: IgeObject, groupName?: string) {
		if (obj !== undefined) {
	...
	sync (method: SyncEntry["method"], attrArr: string | string[]) {
	...
	_processSync () {
		...
		if (this._ige._syncIndex < this._ige._syncArr.length) {
			syncEntry = this._ige._syncArr[this._ige._syncIndex];

			// Add the callback to the last attribute
			syncEntry.attrArr.push(function () {
				this._ige._syncIndex++;
				setTimeout(this._ige._processSync, 1);
			});

			// Call the method
			syncEntry.method.apply(this._ige, syncEntry.attrArr);
		} else {
			// Reached end of sync cycle
			delete this._ige._syncArr;
			delete this._ige._syncIndex;

			this._ige.emit('syncComplete');
		}
	}
	 ...
	 * @param {string} sceneGraphName The name of the scenegraph class.
	 * @param {IgeSceneGraph} sceneGraphInstance The name of the scenegraph class.
	 * @param {Object=} options Optional object to pass to the scenegraph class graph() method.
	 * @returns {*}
	 */
	addGraph (sceneGraphName: string, sceneGraphInstance: IgeSceneGraph, options?) {
			this.log('Loading SceneGraph data class: ' + sceneGraphName);

			// Make sure the graph class implements the required methods "addGraph" and "removeGraph"
			if (typeof(sceneGraphInstance.addGraph) === 'function' && typeof(sceneGraphInstance.removeGraph) === 'function') {
				// Call the class's graph() method passing the options in
				sceneGraphInstance.addGraph(options);

				// Add the graph instance to the holding array
				this._graphInstances[sceneGraphName] = sceneGraphInstance;
			} else {
	 ...
	 * @returns {*}
	 */
	removeGraph (sceneGraphName, options) {
		if (sceneGraphName !== undefined) {
			const sceneGraphInstance = this._graphInstances[sceneGraphName];

			if (sceneGraphInstance) {
				this.log('Removing SceneGraph data class: ' + sceneGraphName);

				// Call the class's graph() method passing the options in
				sceneGraphInstance.removeGraph(options);

				// Now remove the graph instance from the graph instance array
				delete this._graphInstances[sceneGraphName];
			} else {
				this.log('Cannot remove graph for class name "' + sceneGraphName + '" because the class instance could not be found. Did you add it via this._ige.addGraph() ?', 'error');
			}
		}
	...
	debugEnabled (val) {
		if (val !== undefined) {
			if (this._igeConfig.debug) {
				this._igeConfig.debug._enabled = val;
			}
			return this;
		}

		return this._igeConfig.debug._enabled;
	}
	...
	debugTiming (val?: boolean) {
		if (val !== undefined) {
			if (this._igeConfig.debug) {
				this._igeConfig.debug._timing = val;
			}
			return this;
		}

		return this._igeConfig.debug._timing;
	}

	debug (eventName: string) {
		if (this._debugEvents[eventName] === true || this._debugEvents[eventName] === this._ige._frames) {
			debugger;
		}
	}
	...
	triggerDebugEventFrame (eventName: string) {
		this._debugEvents[eventName] = this._ige._frames;
	}
	...
	setFps (fpsRate?: number) {
        ...
        this._requestAnimFrame = (callback, element) => {
            setTimeout(function() {
	...
	updateProgress () {
		...
		if (typeof(document) !== 'undefined' && document.getElementById) {
			...
			if (elem) {
				...
				var totalWidth = (elem.parentNode as HTMLElement).offsetWidth,
	...
	/**
	 * Returns a texture from the texture store by its url.
	 * @param {String} url
	 * @return {IgeTexture}
	 */
	textureFromUrl (url: string): IgeTexture | undefined {
	...
	texturesLoaded ():boolean {
		return this._ige._texturesLoading === 0;
	}
	 ...
	 */
	canvasReady () {
		return (this._ige._canvas !== undefined || this._ige.isServer);
	}
	...
	start (callback) {
        ...
        if (this._ige.dependencyCheck()) {
            // Start the engine
            this._ige.log("Starting engine...");
            this._ige._state = 1;
            ...
            this._requestAnimFrame(this._ige.engineStep);

            this._ige.log("Engine started");
            ...
            if (!this._ige._dependencyCheckStart) {
                this._ige._dependencyCheckStart = curTime;
            }

            // Check if we have timed out
            if (curTime - this._ige._dependencyCheckStart > this._dependencyCheckTimeout) {
	...
	renderContext (contextId) {
		if (contextId !== undefined) {
			...
			this._renderMode = this._renderModes.indexOf(contextId);
	...
	openUrl (url) {
		if (url !== undefined) {
			...
			if (this._ige.cocoonJs && this._ige.cocoonJs.detected) {
				// Open URL via CocoonJS webview
				this._ige.cocoonJs.openUrl(url);
			} else {
	...
	showWebView (url) {
		if (this._ige.cocoonJs && this._ige.cocoonJs.detected) {
			// Open URL via CocoonJS webview
			this._ige.cocoonJs.showWebView(url);
		} else {
			// Load the iFrame url
	...
	hideWebView () {
		if (this._ige.cocoonJs && this._ige.cocoonJs.detected) {
			...
			this._ige.cocoonJs.hideWebView();
		} else {
			var overlay = document.getElementById('igeOverlay');
			if (overlay) {
	 ...
	 * position is set by the this._ige.input component (IgeInputComponent)
	 * @return {IgePoint3d}
	 */
	mousePos () {
	...
	_resizeEvent (event) {
		...
		if (this._ige._autoSize) {
			var newWidth = window.innerWidth,
				newHeight = window.innerHeight,
				arr = this._ige._children,
				arrCount = arr.length;

			// Only update canvas dimensions if it exists
			if (this._ige._canvas) {
				...
				this._ige._canvas.width = newWidth * this._ige._deviceFinalDrawRatio;
				this._ige._canvas.height = newHeight * this._ige._deviceFinalDrawRatio;

				if (this._ige._deviceFinalDrawRatio !== 1) {
					this._ige._canvas.style.width = newWidth + 'px';
					this._ige._canvas.style.height = newHeight + 'px';

					// Scale the canvas context to account for the change
					this._ige._ctx.scale(this._ige._deviceFinalDrawRatio, this._ige._deviceFinalDrawRatio);
				}
			}

			this._ige._bounds2d = new IgePoint3d(newWidth, newHeight, 0);
			...
			if (this._ige._canvas) {
				this._ige._bounds2d = new IgePoint3d(this._ige._canvas.width, this._ige._canvas.height, 0);
			}
		}

		if (this._ige._showSgTree) {
			...
			sgTreeElem.style.height = (this._ige._bounds2d.y - 30) + 'px';
		}

		this._ige._resized = true;
	}
	...
	_canvasPosition () {
		try {
			return this._ige._canvas.getBoundingClientRect();
		} catch (e) {
			return {
				top: this._ige._canvas.offsetTop,
				left: this._ige._canvas.offsetLeft
			};
		}
	}
		...
		Object.defineProperty(obj, propName, {
			...
			set (val) {
				...
				if (obj.___igeTraceCount === obj.___igeTraceMax) {
					...
					// the trace system
					this._ige.traceSetOff(obj, propName);
				}
			}
		});
	}
	...
	traceSetOff (object, propName) {
		Object.defineProperty(object, propName, {set (val) { this.___igeTraceCurrentVal[propName] = val; }});
	}
	 ...
	 * with the this._ige.useManualRender(true) call which will cause the
	...
	engineStep (timeStamp, ctx) {
		...
		if (self._state) {
			...
			unbornQueue = this._ige._spawnQueue;
			...
			for (unbornIndex = unbornCount - 1; unbornIndex >= 0; unbornIndex--) {
				...
				if (this._ige._currentTime >= unbornEntity._bornTime) {
					// Now birth this entity
					unbornEntity.mount(this._ige.$(unbornEntity._birthMount));
			...
			if (self._enableUpdates) {
				if (this._igeConfig.debug._timing) {
					...
					this._ige._updateTime = new Date().getTime() - updateStart;
				} else {
					self.updateSceneGraph(ctx);
				}
			}
			...
			if (self._enableRenders) {
				if (!self._useManualRender) {
					if (this._igeConfig.debug._timing) {
						...
						this._ige._renderTime = new Date().getTime() - renderStart;
					} else {
						self.renderSceneGraph(ctx);
					}
				} else {
	...
	analyseTiming () {
		if (this._igeConfig.debug._timing) {
			...
			this.log('Cannot analyse timing because the this._igeConfig.debug._timing flag is not enabled so no timing data has been recorded!', 'warning');
		}
	}
	...
	sceneGraph (obj, currentDepth, lastDepth) {
		...
		if (this._igeConfig.debug._timing) {
			...
			timingString += 'T: ' + this._ige._timeSpentInTick[obj.id()];
			if (this._ige._timeSpentLastTick[obj.id()]) {
				if (typeof(this._ige._timeSpentLastTick[obj.id()].ms) === 'number') {
					timingString += ' | LastTick: ' + this._ige._timeSpentLastTick[obj.id()].ms;
				}

				if (typeof(this._ige._timeSpentLastTick[obj.id()].depthSortChildren) === 'number') {
					timingString += ' | ChildDepthSort: ' + this._ige._timeSpentLastTick[obj.id()].depthSortChildren;
				}
			}
					...
					if (arr[arrCount]._scene) {
						if (arr[arrCount]._scene._shouldRender) {
							if (this._igeConfig.debug._timing) {
								...
								timingString += 'T: ' + this._ige._timeSpentInTick[arr[arrCount].id()];
								if (this._ige._timeSpentLastTick[arr[arrCount].id()]) {
									if (typeof(this._ige._timeSpentLastTick[arr[arrCount].id()].ms) === 'number') {
										timingString += ' | LastTick: ' + this._ige._timeSpentLastTick[arr[arrCount].id()].ms;
									}

									if (typeof(this._ige._timeSpentLastTick[arr[arrCount].id()].depthSortChildren) === 'number') {
										timingString += ' | ChildDepthSort: ' + this._ige._timeSpentLastTick[arr[arrCount].id()].depthSortChildren;
									}
								}
	...
	_childMounted (child) {
		if (child.IgeViewport) {
			...
			if (!this._ige._currentViewport) {
				this._ige._currentViewport = child;
				this._ige._currentCamera = child.camera;
			}
		}
