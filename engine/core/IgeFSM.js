...
export class IgeFSM extends IgeBaseClass {
    ...
    _transitionStates(oldStateName, newStateName, data, callback) {
        const self = this, currentStateObj = self.getState(self._currentStateName), newStateObj = self.getState(newStateName);
        if (currentStateObj && newStateObj) {
            if (self._debug) {
                self.log("Exiting state: " + self._currentStateName);
            }
            ...
            if (callback) {
                callback("Cannot change states from \"" + self._currentStateName + "\" to \"" + newStateName + "\" states.");
            }
