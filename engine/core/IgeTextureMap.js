...
class IgeTextureMap extends IgeTileMap2d {
    ...
    negate(entity) {
        if (entity !== undefined) {
            var x, y, entityMapData = entity.map._mapData, thisMapData = this.map._mapData;
    ...
    allTexturesLoaded() {
        if (!this._allTexturesLoaded) {
            var arr = this._textureList, arrCount = arr.length;
            while (arrCount--) {
    ...
    saveMap() {
        // in URL format
        var textures = [], i, x, y, dataX = 0, dataY = 0, mapData = this.map._mapData;
    ...
    tick(ctx) {
        ...
        var mapData = this.map._mapData, x, y, tx, ty, xInt, yInt, finalX, finalY, finalPoint, tileData, tileEntity = this._newTileEntity(), // TODO: This is wasteful, cache it?
        sectionX, sectionY, tempSectionX, tempSectionY, _ctx, regions, region, i;
                        ...
                        if (mapData.hasOwnProperty(y)) {
                            for (x in mapData[y]) {
                                if (mapData[y].hasOwnProperty(x)) {
                                    ...
                                    if (tileData) {
                                        regions = this._renderTile(_ctx, xInt, yInt, tileData, tileEntity, null, sectionX, sectionY);
                                        ...
                                        if (regions) {
                                            ...
                                            for (i = 0; i < regions.length; i++) {
                                                ...
                                                if (!this._sectionTileRegion[tempSectionX][tempSectionY][xInt][yInt]) {
                                                    ...
                                                    this._renderTile(_ctx, xInt, yInt, tileData, tileEntity, null, tempSectionX, tempSectionY);
                                                }
                                            }
                                        }
                                    }
    ...
    _drawSectionsToCtx(ctx) {
        var x, y, tileData, sectionRenderX, sectionRenderY, sectionAbsX, sectionAbsY, sectionWidth, sectionHeight, viewArea = this._ige._currentViewport.viewArea();
                    ...
                    if (this._sections[x].hasOwnProperty(y)) {
                        ...
                        if ((sectionAbsX + sectionWidth + (this._tileHeight / 2) >= -(viewArea.width / 2) && sectionAbsX - (this._tileWidth / 2) <= (viewArea.width / 2)) && (sectionAbsY + sectionHeight + (this._tileHeight / 2) >= -(viewArea.height / 2) && sectionAbsY <= (viewArea.height / 2))) {
                            ...
                            if (this._drawSectionBounds) {
                                ...
                                ctx.strokeRect(x * (this._tileWidth * this._autoSection), y * (this._tileHeight * this._autoSection), (this._tileWidth * this._autoSection), (this._tileHeight * this._autoSection));
                            }
                        }
                    }
                }
    ...
    _renderTile(ctx, x, y, tileData, tileEntity, rect, sectionX, sectionY) {
        ...
        var finalX, finalY, regions, xm1, xp1, ym1, yp1, regObj, xAdjust = this._mountMode === 1 ? this._tileWidth / 2 : 0, yAdjust = this._mountMode === 1 ? this._tileHeight / 2 : 0, tx, ty, sx, sy, texture;
        ...
        // Paint the texture
        if (texture) {
            texture.render(ctx, tileEntity, this._ige._tickDelta);
        }
        ctx.restore();
        return regions;
    }
