...
export interface IgeClassProps {
    ige: IgeEngine;
    igeConfig: IgeConfig;
}

export class IgeClass {
    _classId: string;
    componentId?: string;
    _id: string;
    _ige: IgeEngine | null = null;
    _igeConfig: IgeConfig = {
        debug: {
            _enabled: true
        }
    };
     ...
     *     entity.log('An error message', 'error');
     */
    log(text: string, type?, obj?) {
        if (!this._igeConfig.debug._enabled) return this;
        ...
        if (type === "warning" || type === "error") {
            if (this._igeConfig.debug._stacks) {
                if (this._igeConfig.debug._node) {
                    if (console.trace) {
                        console.trace();
                    } else {
                        stack = new Error().stack;
        ...
        if (type === "error") {
            ...
            if (this._igeConfig.debug._throwErrors) {
                throw indent + "IGE *" + type + "* [" + this._classId + thisId + "] : " + text;
            } else {
                console.log(indent + "IGE *" + type + "* [" + this._classId + thisId + "] : " + text);
            }
        } else {
            console.log(indent + "IGE *" + type + "* [" + this._classId + thisId + "] : " + text);
        }
