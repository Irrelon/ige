...
class IgePoly2d {
    ...
    multiply(factor) {
        if (factor !== undefined) {
            var polyPoints = this._poly, pointCount = polyPoints.length, pointIndex;
     ...
     */
    pointInPoly(point) {
        var polyPoints = this._poly, pointCount = polyPoints.length, pointIndex, oldPointIndex = pointCount - 1, c = 0;
    ...
    aabb() {
        var minX, minY, maxX, maxY, xArr = [], yArr = [], arr = this._poly, arrIndex, arrCount = arr.length;
     ...
     */
    clone() {
        var newPoly = new IgePoly2d(), arr = this._poly, arrCount = arr.length, i;
    ...
    clockWiseTriangle() {
        ...
        var arr = this._poly, val, p1, p2, p3;
        p1 = arr[0];
        p2 = arr[1];
        p3 = arr[2];
    ...
    makeClockWiseTriangle() {
        ...
        if (!this.clockWiseTriangle()) {
            var p0 = this._poly[0], p1 = this._poly[1], p2 = this._poly[2];
            this._poly[2] = p1;
            this._poly[1] = p2;
        }
    }
    triangulate() {
        ...
        var poly = this._poly, triangles = [], indices = this.triangulationIndices(), i, point1, point2, point3, newPoly;
    ...
    triangulationIndices() {
        var indices = [], n = this._poly.length, v = [], V = [], nv, count, m, u, w, a, b, c, s, t;
        if (n < 3) {
            return indices;
        }
        if (this._area() > 0) {
    ...
    _area() {
        var n = this._poly.length, a = 0.0, q = 0, p, pval, qval;
        for (p = n - 1; q < n; p = q++) {
            pval = this._poly[p];
            qval = this._poly[q];
    ...
    _snip(u, v, w, n, V) {
        var p, A = this._poly[V[u]], B = this._poly[V[v]], C = this._poly[V[w]], P;
     ...
     */
    render(ctx, fill) {
        var polyPoints = this._poly, pointCount = polyPoints.length, scaleX = this._scale.x, scaleY = this._scale.y, i;
        ctx.beginPath();
