"use strict";
class UiToolBox_ToolSelect extends IgeEventingClass {
    classId;
    init;
}
{
    ...
    ige.editor.on('mouseUp', function (event) {
        ...
        if (event.button === 2 && ige.editor._selectedObject) {
            var classArr = ige.editor._selectedObjectClassList, i;
            for (i = 0; i < classArr.length; i++) {
            }
            var body = $('body'), width = body.width(), height = body.height(), left = event.pageX, top = event.pageY;
            ige.editor.ui.menus.create({
                ...
                var menuWidth = elem.width(), menuHeight = elem.height();
