"use strict";
class UiToolBox_ToolTranslate extends IgeEventingClass {
    classId;
    init;
}
{
...
_mouseDown: function (event) {
    if (!this._opStarted) {
        ...
        var mx = (event.igeX - ige._bounds2d.x2), my = (event.igeY - ige._bounds2d.y2), curMousePos = new IgePoint3d(mx, my, 0);
...
_mouseMove: function (event) {
    if (this._enabled && this._targetEntity) {
        ...
        if (this._opStartMouse) {
            var mx = (event.igeX - ige._bounds2d.x2), my = (event.igeY - ige._bounds2d.y2), curMousePos = { x: mx, y: my }, panCords = {
                x: this._opStartMouse.x - curMousePos.x,
                y: this._opStartMouse.y - curMousePos.y
            }, distX = Math.abs(panCords.x), distY = Math.abs(panCords.y), panFinalX = this._opStartTranslate.x - (panCords.x / ige._currentViewport.camera._scale.x), panFinalY = this._opStartTranslate.y - (panCords.y / ige._currentViewport.camera._scale.y);
            if (this._opPreStart) {
        ...
        if (this._opStarted) {
            if (this._opStartMouse) {
                var mx = (event.igeX - ige._bounds2d.x2), my = (event.igeY - ige._bounds2d.y2), curMousePos = { x: mx, y: my }, panCords = {
                    x: this._opStartMouse.x - curMousePos.x,
                    y: this._opStartMouse.y - curMousePos.y
                }, panFinalX = this._opStartTranslate.x - (panCords.x / ige._currentViewport.camera._scale.x), panFinalY = this._opStartTranslate.y - (panCords.y / ige._currentViewport.camera._scale.y);
