...
}
{
    ...
    self = this;
    ige;
    requireStylesheet(igeRoot + 'components/editor/ui/insert/insert.css');
    ;
    self;
    reset();
}
...
setupListeners: function (dndTarget) {
    var self = this, overFunc, dropFunc;
    ...
    self._canvas.on('click', function (e) {
        var oe = e.originalEvent, cell = self.cellFromXY(oe);
        self.clearCell(cell);
    });
    ...
    dropFunc = function (e) {
        var event = e.originalEvent, dataTransfer = event.dataTransfer, i;
        event.preventDefault();
            ...
            if (dataTransfer.files.length > 1) {
                ...
                ige.editor.ui.dialogs.input({
                    ...
                    positive: function () {
                        ...
                        var columns = $('#multiFileInput').find('select').val(), noBreak = true, i = 0, x = 0, y;
...
getFinalTexture: function () {
    ...
    if (drawnArea.width > 0 && drawnArea.height > 0) {
        ...
        var newCanvas = document.createElement('canvas'), ctx;
...
downloadImage: function () {
    var self = this, drawnArea, form = $('#textureEditorDialog').find('form'), imageDataElem = form.find('#formImageData'), newCanvas;
    // Render a frame without grid lines
    self._renderCanvas(true);
    drawnArea = self.drawnArea();
    if (drawnArea.width > 0 && drawnArea.height > 0) {
...
splitImage: function () {
    var self = this, cols = self._cellCols, rows = self._cellRows;
    ...
    ige.editor.ui.dialogs.input({
        ...
        ready: function () {
            var colsElem = this.find('#cols'), rowsElem = this.find('#rows');
        ...
        positive: function () {
            ...
            var cols = this.find('#cols').val(), rows = this.find('#rows').val();
            cols = cols || 1;
            rows = rows || 1;
            self.cellCols(cols);
            self.cellRows(rows);
...
cellCols: function (val) {
    if (val !== undefined) {
        if (this._backBuffer) {
            // Use the first image
            var img = this._backBuffer, imgWidth = img.width, imgHeight = img.height;
...
resizeBackBuffer: function (newWidth, newHeight) {
    ...
    var newBackBuffer = document.createElement('canvas'), nbbCtx = newBackBuffer.getContext('2d');
...
addImage: function (cell, img) {
    ...
    var self = this, x = cell.x * self._cellWidth, y = cell.y * self._cellHeight;
...
occupyCells: function (x, y, w, h) {
    var self = this, cellX = Math.floor(x / self._cellWidth), cellY = Math.floor(y / self._cellHeight), cellW = Math.floor(w / self._cellWidth), cellH = Math.floor(h / self._cellHeight), j, k;
    for (j = 0; j < cellW; j++) {
        for (k = 0; k < cellH; k++) {
...
clearCell: function (cell) {
    var self = this, j, k, maxX = 0, maxY = 0;
...
_renderCanvas: function (noGrid) {
    var self = this, ctx = self._ctx, cell, cellWidth, cellHeight, j, k;
    // Clear the canvas
