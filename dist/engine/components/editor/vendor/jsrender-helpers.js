...
if (jsviews) {
    jsviews.helpers({
        ...
        collapsePost: function (msg) {
            if (msg) {
                if (msg.length <= 1000) {
                    var re = new RegExp('{-img data-app-type="post"-}', "g"), matchArr = msg.match(re);
            ...
            if (msg) {
                if (postImages && postImages.length) {
                    ...
                    for (var i = 0; i < postImages.length; i++) {
                        ...
                        msg = msg.replace('{-img data-app-type="post"-}', '<a href="' +
                            postImages[i].url +
                            originalTag +
                            '" data-app-static="1" target="_blank"><img src="' +
                            postImages[i].url +
                            sizeTag +
                            '" width="300" /></a>');
                    }
        ...
        convertUrls: function (msg) {
            if (msg) {
                var hyperlinkExp = /(http|ftp|https):\/\/[\w-]+(\.[\w-]+)+(?![^\s]*?")([\w.,@?^=%&amp;:\/~+#-]*[\w@?^=%&amp;\/~+#-])?/gi, results, replace = [], i, imgUrl;
                msg = msg.replace(/\n/gi, "<br />");
                results = msg.match(hyperlinkExp);
                if (results) {
                    for (i = 0; i < results.length; i++) {
                        if (results[i].search(/www\.youtube\.com\/watch/gi) > -1) {
                            ...
                            // Embed video
                            msg = msg.replace(results[i], '<iframe width="350" height="263" src="http://www.youtube-nocookie.com/embed/' +
                                args[1] +
                            ...
                            msg = msg.replace(results[i], '<a href="' +
                                results[i] +
                                '" target="_blank" data-app-static="1">' +
                                decodeURI(results[i]) +
                                "</a>");
                        }
                    }
