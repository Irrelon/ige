...
(function (global, jQuery, undefined) {
    ...
    //========================== Top-level vars ==========================
    var versionNumber = "v1.0.0-beta", $, jsvStoreName, rTag, rTmplString, // nodeJsModule,
    //TODO	tmplFnsCache = {},
    delimOpenChar0 = "{", delimOpenChar1 = "{", delimCloseChar0 = "}", delimCloseChar1 = "}", linkChar = "^", rPath = /^(!*?)(?:null|true|false|\d[\d.]*|([\w$]+|\.|~([\w$]+)|#(view|([\w$]+))?)([\w$.^]*?)(?:[.[^]([\w$]+)\]?)?)$/g, 
    //                                     none   object     helper    view  viewProperty pathTokens      leafToken
    rParams = /(\()(?=\s*\()|(?:([([])\s*)?(?:(\^?)(!*?[#~]?[\w$.^]+)?\s*((\+\+|--)|\+|-|&&|\|\||===|!==|==|!=|<=|>=|[<>%*:?\/]|(=))\s*|(!*?[#~]?[\w$.^]+)([([])?)|(,\s*)|(\(?)\\?(?:(')|("))|(?:\s*(([)\]])(?=\s*\.|\s*\^|\s*$)|[)\]])([([]?))|(\s+)/g, 
    //          lftPrn0        lftPrn        bound            path    operator err                                                eq             path2       prn    comma   lftPrn2   apos quot      rtPrn rtPrnDot                        prn2      space
    // (left paren? followed by (path? followed by operator) or (path followed by left paren?)) or comma or apos or quot or right paren or space
    rNewLine = /[ \t]*(\r\n|\n|\r)/g, rUnescapeQuotes = /\\(['"])/g, rEscapeQuotes = /['"\\]/g, // Escape quotes and \ character
    rBuildHash = /\x08(~)?([^\x08]+)\x08/g, rTestElseIf = /^if\s/, rFirstElem = /<(\w+)[>\s]/, rAttrEncode = /[\x00`><"'&]/g, // Includes > encoding since rConvertMarkers in JsViews does not skip > characters in attribute strings
    rHtmlEncode = rAttrEncode, autoTmplName = 0, viewId = 0, charEntities = {
        "&": "&amp;",
        "<": "&lt;",
        ">": "&gt;",
        "\x00": "&#0;",
        ...
        '"': "&#34;",
        "`": "&#96;"
    }, tmplAttr = "data-jsv-tmpl", $render = {}, jsvStores = {
        template: {
            compile: compileTmpl
        },
        tag: {
    ...
    function convertVal(converter, view, tagCtx) {
        ...
        var tmplConverter, tag, value, boundTagCtx = +tagCtx === tagCtx && tagCtx, // if tagCtx is an integer, then it is the key for the boundTagCtx (compiled function to return the tagCtx)
        ...
        if (converter || boundTagCtx) {
            ...
            if (converter &&
                ((tmplConverter = view.getRsc("converters", converter)) ||
                    error("Unknown converter: {{" + converter + ":"))) {
    ...
    function renderTag(tagName, parentView, tmpl, tagCtxs, isRefresh) {
        ...
        var render, tag, tags, attr, parentTag, i, l, itemRet, tagCtx, tagCtxCtx, content, boundTagFn, tagDef, callInit, ret = "", boundTagKey = +tagCtxs === tagCtxs && tagCtxs, // if tagCtxs is an integer, then it is the boundTagKey
        linkCtx = parentView.linkCtx || 0, ctx = parentView.ctx, parentTmpl = tmpl || parentView.tmpl, parentView_ = parentView._;
        if (tagName._is === "tag") {
            tag = tagName;
            tagName = tag.tagName;
        }
                ...
                tags =
                    tag.parents =
                        tagCtxCtx.parentTags =
                            (ctx && extendCtx(tagCtxCtx.parentTags, ctx.parentTags)) || {};
                if (parentTag) {
    ...
    function View(context, type, parentView, data, template, key, contentTmpl, onRender) {
        ...
        if (parentView) {
            ...
            if (parentView_.useKey) {
                ...
                (self_.key = self.index = key), 0, self);
            }
                    ...
                    elem =
                        value.nodeType > 0
                            ...
                            : !rTmplString.test(value) &&
                                ...
                                jQuery &&
                                jQuery(global.document).find(value)[0];
    ...
    function renderContent(data, context, parentView, key, isLayout, onRender) {
        ...
        if (self.tag) {
            ...
            tmpl =
                (self.jquery && (self[0] || error('Unknown template: "' + self.selector + '"'))) || // This is a call from $(selector).render
                    self;
        }
        if (tmpl) {
                ...
                if ($.isArray(data) && !isLayout) {
                    ...
                    newView = swapContent
                        ...
                        : (key !== undefined && parentView) ||
                            View(context, "array", parentView, data, tmpl, key, contentTmpl, onRender);
                    ...
                    newView = swapContent
                        ? parentView
                        : View(context, tmplName || "data", parentView, data, tmpl, key, contentTmpl, onRender);
                    if (tag_ && !self.flow) {
                        newView.tag = self;
                    }
    ...
    function tmplFn(markup, tmpl, isLinkExpr, convertBack) {
        ...
        function parseTag(all, bind, tagName, converter, colon, html, comment, codeTag, params, slash, closeBlock, index) {
            ...
            var noError, current0, pathBindings = bind && [], code = "", hash = "", passedCtx = "", 
    ...
    function buildCode(ast, tmpl, isLinkExpr) {
        ...
        // Used for compiling templates, and also by JsViews to build functions for data link expressions
        var i, node, tagName, converter, params, hash, hasTag, hasEncoder, getsVal, hasCnvt, useCnvt, tmplBindings, pathBindings, nestedTmpls, tmplName, nestedTmpl, tagAndElses, content, markup, nextIsElse, oldCode, isElse, isGetVal, prm, tagCtxFn, tmplBindingKey = 0, code = "", noError = "", tmplOptions = {}, l = ast.length;
        if ("" + tmpl === tmpl) {
    ...
    function parseParams(params, bindings, tmpl) {
        ...
        function parseTokens(all, lftPrn0, lftPrn, bound, path, operator, err, eq, path2, prn, comma, lftPrn2, apos, quot, rtPrn, rtPrnDot, prn2, space, index, full) {
            ...
            function parsePath(allPath, not, object, helper, view, viewProperty, pathTokens, leafToken) {
                ...
                if (object) {
                    ...
                    if (object !== ".") {
                        var ret = (helper ? 'view.hlp("' + helper + '")' : view ? "view" : "data") +
                            (leafToken
                                ? (viewProperty ? "." + viewProperty : helper ? "" : view ? "" : "." + object) +
                                    (pathTokens || "")
                                : ((leafToken = helper ? "" : view ? viewProperty || "" : object), ""));
                        ret = ret + (leafToken ? "." + leafToken : "");
                        return (not +
                            (ret.slice(0, 9) === "view.data"
                ...
                if (bindings && rtPrnDot && !aposed && !quoted) {
                    ...
                    if (!named || boundName || bindto) {
                        ...
                        if (full.length - 1 > index - expr) {
                            ...
                            rtPrnDot = tmplLinks[rtPrnDot] =
                                tmplLinks[rtPrnDot] || tmplFn(delimOpenChar0 + rtPrnDot + delimCloseChar1, tmpl, true); // Compile the expression (or use cached copy already in tmpl.links)
                            if (!rtPrnDot.paths) {
        ...
        tmplLinks = tmpl.links, fnCall = {}, pathStart = { 0: -1 }, parenDepth = 0, quoted = false, // boolean for string content in double quotes
    ...
    var $templates = $views.templates, $converters = $views.converters, $helpers = $views.helpers, $tags = $views.tags, $viewsSub = $views.sub, $viewsSettings = $views.settings, noIndex = "Error: #index in nested view: use #getIndex()"; // Error string if debugMode, else empty
    if (jQuery) {
        ////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    $tags({
        ...
        if: {
            render: function (val) {
                ...
                // Otherwise return ""
                var self = this, ret = self.rendering.done || (!val && (arguments.length || !self.tagCtx.index))
                    ? ""
                    : ((self.rendering.done = true),
                        (self.selected = self.tagCtx.index),
    ...
    function renderForBlock(val) {
        ...
        // We will use the tag.rendering object for carrying rendering state across the calls.
        var self = this, tagCtx = self.tagCtx, noArg = !arguments.length, result = "", done = noArg || 0;
