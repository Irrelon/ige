        ...
        ige.on("texturesLoaded", function () {
            ...
            ige.start(function (success) {
                ...
                if (success) {
                    ...
                    ige.$("vp1").addComponent(IgeMouseZoomComponent).mouseZoom.enabled(true);
                    // Create the UI scene
                    self.uiScene = new IgeScene2d().id("uiScene").depth(1).ignoreCamera(true).mount(ige.$("baseScene"));
                    ige.ui.style("#topNav", {
                    ...
                    var topNav = new IgeUiElement().id("topNav").mount(self.uiScene);
                    new IgeUiElement().id("logo").mount(topNav);
                    var leftNav = new IgeUiElement().id("leftNav").mount(self.uiScene);
                    var main = new IgeUiElement().id("main").mount(self.uiScene);
                    new IgeUiDropDown()
                        .id("optionsDropDown")
                        .top(10)
                        .left(10)
                        ...
                        .mount(leftNav);
                    new IgeUiLabel().value("Dashboard").styleClass("title").mount(main);
                    new IgeUiLabel().value("Login with your username and password").styleClass("subTitle").mount(main);
                    var dashBar = new IgeUiElement().id("dashBar").mount(main);
                    new IgeUiLabel().id("homeLabel").value("Home").width(100).height(40).left(0).top(0).mount(dashBar);
                    new IgeUiTextBox()
                        .id("textBox1")
                        .value("")
                    ...
                    new IgeUiTextBox()
                        ...
                        .placeHolderColor("#989898")
                        .mount(main);
                    new IgeUiButton().id("button1").value("Submit").mount(main);
                }
            });
        });
    }
