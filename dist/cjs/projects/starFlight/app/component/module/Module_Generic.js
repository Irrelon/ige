					...
					switch (audioDefinition.action) {
						case "play":
							if (audioDefinition.for === "all") {
								sourceAudio[audioDefinition.audioId] = new index_1.IgeAudioEntity(
									audioDefinition.audioId,
	...
	resolve(states, tickDelta) {
				...
				if (this._definition.input.hasOwnProperty(inputId)) {
					switch (inputId) {
						...
						default:
							...
							const modifierCalcData = (0, services_1.calculateModifierRatio)(
								states,
								this._definition.input[inputId],
								states[inputId].min,
								states[inputId].max,
								tickDelta,
								inputId
				...
				if (this._definition.output.hasOwnProperty(outputId)) {
					switch (outputId) {
						...
						default:
							...
							const modifierCalcData = (0, services_1.calculateModifierRatio)(
								states,
								this._definition.output[outputId],
								states[outputId].min,
								states[outputId].max,
								tickDelta,
								outputId
		...
		for (const inputId in inputValues) {
			if (inputValues.hasOwnProperty(inputId)) {
				...
				states[inputId].val = (0, services_1.roundNumber)(
					states[inputId].val + inputValues[inputId] * currentRatio,
					6
				);
			}
		}
		for (const outputId in outputValues) {
			if (outputValues.hasOwnProperty(outputId)) {
				// Assign the new state value
				states[outputId].val = (0, services_1.roundNumber)(
					states[outputId].val + outputValues[outputId] * currentRatio,
					6
				);
			}
		}
