...
appCore.module('StationClient', function ($ige, $game, IgeStreamComponent) {
    ...
    StationClient.prototype._onPlayerEntity = function (entityId) {
        let self = this, eventListener, ent = instance_1.ige.engine.$(entityId);
        if (ent) {
        ...
        // should be tracking!
        eventListener = instance_1.ige.network.stream.on('entityCreated', function (entity) {
            if (entity.id() === entityId) {
                self._trackPlayerEntity(instance_1.ige.engine.$(entityId));
                // Turn off the listener for this event now that we
                // have found and started tracking our player entity
                instance_1.ige.network.stream.off('entityCreated', eventListener, function (result) {
                    if (!result) {
    ...
    StationClient.prototype._trackPlayerEntity = function (ent) {
        ...
        instance_1.ige.$("vp1").camera.trackTranslate(ent, 8);
