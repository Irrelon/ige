...
const createStorageBuilding = (parent, id, tileX, tileY) => {
    const roadOffsetX = 0;
    const roadOffsetY = 3;
    const newBuilding = new StorageBuilding_1.StorageBuilding(tileX, tileY).id(id).mount(parent).translateToTile(tileX, tileY, 0);
...
const createHouseBuilding1 = (parent, id, tileX, tileY) => {
    const roadOffsetX = 0;
    const roadOffsetY = 2;
    const newBuilding = new HouseBuilding1_1.HouseBuilding1(tileX, tileY, ResourceType_1.ResourceType.gold, [
        { count: 1, type: ResourceType_1.ResourceType.science, max: 1 },
        { count: 1, type: ResourceType_1.ResourceType.energy, max: 1 }
    ])
        .id(id)
        .mount(parent)
        .translateToTile(tileX, tileY, 0);
...
const createFlagBuilding = (parent, id, tileX, tileY) => {
    const newBuilding = new FlagBuilding_1.FlagBuilding(tileX, tileY).id(id).mount(parent).translateToTile(tileX, tileY, 0);
    return newBuilding;
};
