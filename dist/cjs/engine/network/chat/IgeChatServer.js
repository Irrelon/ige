...
class IgeChatServer extends IgeChatComponent_1.IgeChatComponent {
    constructor() {
        ...
        instance_1.ige.dependencies.waitFor(["network"], () => {
            ...
            network.define(IgeNetworkConstants_1.IGE_NETWORK_CHAT_MSG, this._onMessageFromClient);
            network.define(IgeNetworkConstants_1.IGE_NETWORK_CHAT_JOIN_ROOM, this._onJoinRoomRequestFromClient);
            network.define(IgeNetworkConstants_1.IGE_NETWORK_CHAT_LEAVE_ROOM, this._onLeaveRoomRequestFromClient);
            network.define(IgeNetworkConstants_1.IGE_NETWORK_CHAT_LIST_ROOMS, this._onClientWantsRoomList);
            network.define(IgeNetworkConstants_1.IGE_NETWORK_CHAT_ROOM_LIST_USERS, this._onClientWantsRoomUserList);
            network.define(IgeNetworkConstants_1.IGE_NETWORK_CHAT_ROOM_CREATED);
            network.define(IgeNetworkConstants_1.IGE_NETWORK_CHAT_ROOM_REMOVED);
    ...
    createRoom(roomName, roomId, options) {
        const network = instance_1.ige.network;
        const newRoomId = roomId || (0, utils_1.newIdHex)();
        ...
        network.send(IgeNetworkConstants_1.IGE_NETWORK_CHAT_ROOM_CREATED, newRoomId);
        return roomId;
    }
    /**
    ...
    removeRoom(roomId) {
        ...
        if (this._rooms[roomId]) {
            ...
            network.send(IgeNetworkConstants_1.IGE_NETWORK_CHAT_ROOM_REMOVED, roomId);
            ...
            if (message !== undefined) {
                ...
                if (to) {
                    ...
                    if (room.users.indexOf(to) > -1) {
                        network.send(IgeNetworkConstants_1.IGE_NETWORK_CHAT_MSG, msg, to);
                    }
                    else {
                    ...
                    network.send(IgeNetworkConstants_1.IGE_NETWORK_CHAT_MSG, msg, room.users);
                }
            }
            else {
    ...
    _onMessageFromClient(msg, clientId) {
        ...
        if (this.emit('messageFromClient', msg, clientId) !== IgeEventReturnFlag_1.IgeEventReturnFlag.cancel) {
    ...
    _onJoinRoomRequestFromClient(roomId, clientId) {
        ...
        // Emit the event and if it wasn't cancelled (by returning true) then
        // process this ourselves
        if (this.emit('clientJoinRoomRequest', roomId, clientId) !== IgeEventReturnFlag_1.IgeEventReturnFlag.cancel) {
            const room = this._rooms[roomId];
            ...
            if (room) {
                ...
                if (room.users.indexOf(clientId) === -1) {
                    ...
                    network.send(IgeNetworkConstants_1.IGE_NETWORK_CHAT_JOIN_ROOM, { roomId: roomId, joined: true }, clientId);
    ...
    _onLeaveRoomRequestFromClient(roomId, clientId) {
        // Emit the event and if it wasn't cancelled (by returning true) then
        // process this ourselves
        if (this.emit('clientLeaveRoomRequest', roomId, clientId) !== IgeEventReturnFlag_1.IgeEventReturnFlag.cancel) {
    ...
    _onClientWantsRoomList(data, clientId) {
        // Emit the event and if it wasn't cancelled (by returning true) then
        // process this ourselves
        if (this.emit('clientRoomListRequest', data, clientId) !== IgeEventReturnFlag_1.IgeEventReturnFlag.cancel) {
    ...
    _onClientWantsRoomUserList(roomId, clientId) {
        // Emit the event and if it wasn't cancelled (by returning true) then
        // process this ourselves
        if (this.emit('clientRoomUserListRequest', roomId, clientId) !== IgeEventReturnFlag_1.IgeEventReturnFlag.cancel) {
