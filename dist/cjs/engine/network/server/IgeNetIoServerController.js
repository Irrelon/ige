...
class IgeNetIoServerController extends IgeNetIoBaseController_1.IgeNetIoBaseController {
	constructor() {
		...
		this._onClientConnect = (socket) => {
			if (!this._acceptConnections) {
				this.log(
					`Rejecting connection with id "${socket._id}": We are not accepting connections at the moment!`
				);
				socket.close();
				return;
			}
			// Check if any listener cancels this
			if (this.emit("connect", socket) === IgeEventReturnFlag_1.IgeEventReturnFlag.cancel) {
			...
			for (const entityId in queueObj) {
				...
				item[1].forEach((clientId) => {
					...
					if (!hasSentTimeDataByClientId[clientId]) {
						...
						network.send(IgeNetworkConstants_1.IGE_NETWORK_STREAM_TIME, currentTime, clientId);
						hasSentTimeDataByClientId[clientId] = true;
					}
					network.send(IgeNetworkConstants_1.IGE_NETWORK_STREAM_DATA, item[0], clientId);
		...
		this.define(IgeNetworkConstants_1.IGE_NETWORK_STREAM_CREATE);
		this.define(IgeNetworkConstants_1.IGE_NETWORK_STREAM_DESTROY);
		this.define(IgeNetworkConstants_1.IGE_NETWORK_STREAM_DATA);
		this.define(IgeNetworkConstants_1.IGE_NETWORK_STREAM_TIME);
	}
	/**
	...
	start(port, callback) {
		return new Promise((resolve) => {
			...
			this.define(IgeNetworkConstants_1.IGE_NETWORK_REQUEST, this._onRequest);
			this.define(IgeNetworkConstants_1.IGE_NETWORK_RESPONSE, this._onResponse);
			this.define(IgeNetworkConstants_1.IGE_NETWORK_TIME_SYNC, this._onTimeSync);
			// Start network sync
			this.timeSyncStart();
	...
	clientLeaveRoom(clientId, roomId) {
		if (this._clientRooms[clientId]) {
			(0, utils_1.arrPull)(this._clientRooms[clientId], roomId);
	...
	send(commandName, data, clientIdOrArrayOfIds, callback) {
		...
		if (commandIndex !== undefined) {
			...
			(_a = this._io) === null || _a === void 0
				? void 0
				: _a.send([encodedCommandIndex, data], clientIdOrArrayOfIds);
			return this;
		}
		this.log(
	...
	request(cmd, data, clientIdOrArrayOfIds, callback) {
		...
		this.send(
			IgeNetworkConstants_1.IGE_NETWORK_REQUEST,
			{
				id: req.id,
				cmd,
				data: req.data
	...
	response(requestId, data) {
		...
		this.send(
			IgeNetworkConstants_1.IGE_NETWORK_RESPONSE,
			{
				id: requestId,
				cmd: req.cmd,
				data
	...
	_sendTimeSync(clientId) {
		...
		this.send(IgeNetworkConstants_1.IGE_NETWORK_TIME_SYNC, data, clientId);
	}
	/**
...
exports.IgeNetIoServerController = IgeNetIoServerController;
