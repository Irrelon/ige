...
exports.igeGltfLoader = exports.IgeGltfLoader = void 0;
const IgeBaseClass_1 = require("../core/IgeBaseClass.js");
const IgeEntity_1 = require("../core/IgeEntity.js");
const IgeWebGlMaterial_1 = require("./IgeWebGlMaterial.js");
/**
 ...
 */
class IgeGltfLoader extends IgeBaseClass_1.IgeBaseClass {
    constructor() {
        super(...arguments);
        this.classId = "IgeGltfLoader";
        // Cache loaded models
    ...
    load(url, modelId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (isGlb) {
                ...
                const response = yield fetch(url);
                gltfData = yield response.json();
            }
            // Get base URL for relative paths
            const baseUrl = url.substring(0, url.lastIndexOf("/") + 1);
            // Load binary buffers if not GLB
            const buffers = yield this._loadBuffers(gltfData, baseUrl, binaryData);
            // Parse the GLTF data
            const model = yield this._parseGltf(gltfData, buffers, baseUrl, id);
            // Cache and return
     ...
     */
    _loadBuffers(gltf, baseUrl, embeddedBinary) {
        return __awaiter(this, void 0, void 0, function* () {
            const buffers = [];
            if (!gltf.buffers)
                return buffers;
            for (let i = 0; i < gltf.buffers.length; i++) {
                ...
                if (embeddedBinary && i === 0 && !bufferDef.uri) {
                    ...
                    if (bufferDef.uri.startsWith("data:")) {
                        ...
                        // External file
                        const response = yield fetch(baseUrl + bufferDef.uri);
                        buffers.push(yield response.arrayBuffer());
                    }
                }
            }
            return buffers;
    ...
    _parseMaterials(gltf, textures, images) {
        ...
        if (!gltf.materials) {
            ...
            const defaultMat = new IgeWebGlMaterial_1.IgeWebGlMaterial("default");
            defaultMat.color(0.8, 0.8, 0.8, 1);
            materials.push(defaultMat);
            return materials;
        }
        for (const matDef of gltf.materials) {
            const material = new IgeWebGlMaterial_1.IgeWebGlPBRMaterial(matDef.name);
            ...
            if (matDef.pbrMetallicRoughness) {
                ...
                if (pbr.baseColorFactor) {
                    material.color(pbr.baseColorFactor[0], pbr.baseColorFactor[1], pbr.baseColorFactor[2], (_a = pbr.baseColorFactor[3]) !== null && _a !== void 0 ? _a : 1);
                }
                // Base color texture
                if (pbr.baseColorTexture !== undefined) {
                    const textureIndex = pbr.baseColorTexture.index;
                    const texCoord = (_b = pbr.baseColorTexture.texCoord) !== null && _b !== void 0 ? _b : 0;
    ...
    createEntity(model, sceneIndex) {
        const root = new IgeEntity_1.IgeEntity();
        root.id(model.id);
        // Get the scene to instantiate
        const scene = model.scenes[sceneIndex !== null && sceneIndex !== void 0 ? sceneIndex : model.defaultSceneIndex];
        if (!scene) {
...
exports.IgeGltfLoader = IgeGltfLoader;
// Export singleton instance
exports.igeGltfLoader = new IgeGltfLoader();
