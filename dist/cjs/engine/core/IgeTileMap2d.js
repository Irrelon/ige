...
const IgeTileMap2dSmartTexture_1 = require("../textures/IgeTileMap2dSmartTexture");
...
class IgeTileMap2d extends IgeEntity_1.IgeEntity {
	constructor(tileWidth, tileHeight) {
		...
		if (!clientServer_1.isServer) {
			const tex = new IgeTexture_1.IgeTexture(
				(0, utils_1.newIdHex)(),
				IgeTileMap2dSmartTexture_1.IgeTileMap2dSmartTexture
			);
			this.texture(tex);
		}
	...
	pointToTile(point) {
		...
		if (this._mountMode === IgeMountMode_1.IgeMountMode.flat) {
			...
			tilePos = new IgePoint3d_1.IgePoint3d(
	...
	tileToPoint(x, y) {
		...
		if (this._mountMode === IgeMountMode_1.IgeMountMode.flat) {
			...
			point = new IgePoint3d_1.IgePoint3d(
	...
	scanRects(callback) {
		const rectArray = [];
		const mapData = (0, utils_1.arrClone)(this.map._mapData);
	...
	_updateAdjustmentMatrix() {
		...
		if (this._bounds2d.x2 && this._bounds2d.y2 && this._tileWidth && this._tileHeight) {
			if (this._mountMode === IgeMountMode_1.IgeMountMode.flat) {
				(_a = this._adjustmentMatrix) === null || _a === void 0
					? void 0
					: _a.translateTo(this._bounds2d.x2, this._bounds2d.y2);
			}
			if (this._mountMode === IgeMountMode_1.IgeMountMode.iso) {
