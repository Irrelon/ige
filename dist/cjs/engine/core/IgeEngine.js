...
class IgeEngine extends exports_2.IgeEntity {
    constructor() {
        super();
        ...
        this._state = exports_8.IgeEngineState.stopped;
        ...
        this._resizeEvent = (event) => {
            var _a, _b, _c;
            if (this._renderer) {
                (_a = this._renderer) === null || _a === void 0 ? void 0 : _a._resizeEvent(event);
                this._bounds2d = (_b = this._renderer) === null || _b === void 0 ? void 0 : _b._bounds2d;
            }
            ...
            if (this._autoSize) {
                ...
                if (this._canvas) {
                    ...
                    if (this._devicePixelRatio !== 1) {
                        ...
                        (_c = this._ctx) === null || _c === void 0 ? void 0 : _c.scale(this._devicePixelRatio, this._devicePixelRatio);
                    }
                }
         ...
         */
        this.canvasReady = () => {
            return this._canvas !== undefined || (this._renderer && this._renderer.isReady()) || exports_9.isWorker || exports_9.isServer;
        };
        /**
        ...
        this.engineStep = (timeStamp, ctx) => {
            ...
            if (this._state === exports_8.IgeEngineState.started) {
                ...
                this._processBehaviours(exports_7.IgeBehaviourType.postTick, { ctx, tickDelta });
        ...
        this._viewportDepth = false;
        this._pointerPos = new exports_4.IgePoint3d(0, 0, 0);
        ...
        this._globalScale = new exports_4.IgePoint3d(1, 1, 1);
        ...
        this._ctx = new exports_1.IgeDummyContext();
        this._headless = true;
    ...
    currentViewport(viewport) {
        if (viewport instanceof exports_5.IgeViewport) {
            exports_6.ige.engine._currentViewport = viewport;
            exports_6.ige.engine._currentCamera = viewport.camera;
        }
        return exports_6.ige.engine._currentViewport;
    }
    renderer(renderer) {
    ...
    canvas(elem, autoSize = true) {
        ...
        exports_6.ige.input.setupListeners(this._canvas);
    }
    /**
    ...
    removeCanvas() {
        ...
        if (exports_6.ige.input) {
            exports_6.ige.input.destroyListeners();
        }
    ...
    debugEnabled(val) {
        if (val !== undefined) {
            if (exports_6.ige.config.debug) {
                exports_6.ige.config.debug._enabled = val;
            }
            return this;
        }
        return exports_6.ige.config.debug._enabled;
    }
    /**
    ...
    debugTiming(val) {
        if (val !== undefined) {
            if (exports_6.ige.config.debug) {
                exports_6.ige.config.debug._timing = val;
            }
            return this;
        }
        return exports_6.ige.config.debug._timing;
    }
    debug(eventName) {
     ...
     */
    hideAllExcept(id) {
        const arr = exports_6.ige.register.all();
    ...
    getClass(id) {
        ...
        return exports_6.ige.classStore[id];
    }
    /**
     ...
     */
    classDefined(id) {
        return Boolean(exports_6.ige.classStore[id]);
    }
    /**
    ...
    updateProgress() {
        ...
        const currentWidth = Math.floor((totalWidth / exports_6.ige.textures._assetsTotal) * (exports_6.ige.textures._assetsTotal - exports_6.ige.textures._assetsLoading));
        ...
        textElem.innerHTML =
            this._loadingPreText +
                " " +
                Math.floor((100 / exports_6.ige.textures._assetsTotal) * (exports_6.ige.textures._assetsTotal - exports_6.ige.textures._assetsLoading)) +
                "%";
    }
    /**
    ...
    start() {
        return new Promise((resolve, reject) => {
            ...
            if (exports_9.isClient && this._dependencyQueue.length === 0) {
                // Add the textures loaded dependency
                this._dependencyQueue.push(exports_6.ige.textures.haveAllTexturesLoaded);
     ...
     */
    mousePos() {
        return exports_6.ige._pointerPos.clone();
    }
    _childMounted(child) {
        if (child instanceof exports_5.IgeViewport) {
            // The first mounted viewport gets set as the current
            // one before any rendering is done
            if (!exports_6.ige.engine._currentViewport) {
                exports_6.ige.engine.currentViewport(child);
            }
        }
                ...
                while (arrCount--) {
                    ...
                    if (arr[arrCount]) {
                        if (!exports_6.ige.engine._timeSpentInUpdate[arr[arrCount].id()]) {
                            exports_6.ige.engine._timeSpentInUpdate[arr[arrCount].id()] = 0;
                        }
                        if (!exports_6.ige.engine._timeSpentLastUpdate[arr[arrCount].id()]) {
                            exports_6.ige.engine._timeSpentLastUpdate[arr[arrCount].id()] = {};
                        }
                        exports_6.ige.engine._timeSpentInUpdate[arr[arrCount].id()] += ud;
                        exports_6.ige.engine._timeSpentLastUpdate[arr[arrCount].id()].ms = ud;
                    }
                }
            }
            else {
    ...
    renderSceneGraph(ctx) {
        ...
        const tickDelta = exports_6.ige.engine._tickDelta;
        // Process any behaviours assigned to the engine
        this._processBehaviours(exports_7.IgeBehaviourType.preTick, { ctx, tickDelta });
        ...
        if (this._viewportDepth) {
            if (exports_6.ige.config.debug._timing) {
                ...
                if (!exports_6.ige.engine._timeSpentLastTick[this.id()]) {
                    exports_6.ige.engine._timeSpentLastTick[this.id()] = {};
                }
                exports_6.ige.engine._timeSpentLastTick[this.id()].depthSortChildren = td;
            }
            else {
                this.depthSortChildren();
            }
        ...
        //
        // ctx.restore();
        (_a = this._renderer) === null || _a === void 0 ? void 0 : _a.renderSceneGraph(this._children, this._bounds2d);
    }
    destroy() {
