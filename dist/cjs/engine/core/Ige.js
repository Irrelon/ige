	...
	constructor() {
		...
		if (clientServer_1.isClient) {
			this.dependencies.add(
				"network",
				Promise.resolve()
					.then(() => __importStar(require("../network/client/IgeNetIoClientController.js")))
					.then(({ IgeNetIoClientController: Module }) => {
			...
			if (!clientServer_1.isWorker) {
				this.dependencies.add(
					"audio",
					Promise.resolve()
						.then(() => __importStar(require("../audio/IgeAudioController.js")))
						.then(({ IgeAudioController: Module }) => {
		...
		if (clientServer_1.isServer) {
			this.dependencies.add(
				"network",
				Promise.resolve()
					.then(() => __importStar(require("../network/server/IgeNetIoServerController.js")))
					.then(({ IgeNetIoServerController: Module }) => {
						this.network = new Module();
					})
			);
		}
