...
class IgeObject extends IgeEventingClass_1.IgeEventingClass {
    ...
    screenPosition() {
        ...
        return new IgePoint3d_1.IgePoint3d(Math.floor((this._worldMatrix.matrix[2] - instance_1.ige.engine._currentCamera._translate.x) * instance_1.ige.engine._currentCamera._scale.x + instance_1.ige.engine._bounds2d.x2), Math.floor((this._worldMatrix.matrix[5] - instance_1.ige.engine._currentCamera._translate.y) * instance_1.ige.engine._currentCamera._scale.y + instance_1.ige.engine._bounds2d.y2), 0);
    }
    /**
    ...
    localBounds3dPolygon(recalculate = false) {
        if (this._bounds3dPolygonDirty || !this._localBounds3dPolygon || recalculate) {
            const geom = this._bounds3d, poly = new IgePoly2d_1.IgePoly2d(), 
            // Bottom face
            bf2 = (0, utils_1.toIso)(+geom.x2, -geom.y2, -geom.z2), bf3 = (0, utils_1.toIso)(+geom.x2, +geom.y2, -geom.z2), bf4 = (0, utils_1.toIso)(-geom.x2, +geom.y2, -geom.z2), 
            // Top face
            tf1 = (0, utils_1.toIso)(-geom.x2, -geom.y2, geom.z2), tf2 = (0, utils_1.toIso)(+geom.x2, -geom.y2, geom.z2), tf4 = (0, utils_1.toIso)(-geom.x2, +geom.y2, geom.z2);
    ...
    cache(val, propagateToChildren = false) {
        ...
        if (clientServer_1.isClient) {
            // Use a real canvas
            const canvasObj = instance_1.ige.engine.createCanvas({ smoothing: this._cacheSmoothing, pixelRatioScaling: true });
    ...
    sendStreamCreate(clientId) {
        ...
        network.send(IgeNetworkConstants_1.IGE_NETWORK_STREAM_CREATE, [
    ...
    streamDestroy(clientId) {
        ...
        network.send(IgeNetworkConstants_1.IGE_NETWORK_STREAM_DESTROY, [instance_1.ige.engine._currentTime, thisId], clientId);
        network._streamClientCreated[thisId] = network._streamClientCreated[thisId] || {};
        network._streamClientData[thisId] = network._streamClientData[thisId] || {};
        if (clientId) {
