...
class IgeTexture extends IgeAsset_1.IgeAsset {
    ...
    constructor(id, urlOrObject) {
        ...
        this._sizeX = 0;
        this._sizeY = 0;
        this._renderMode = IgeTextureRenderMode_1.IgeTextureRenderMode.none;
        this._loaded = false;
        ...
        if (!instance_1.ige.textures._textureImageStore[imageUrl]) {
            fetch(imageUrl)
                ...
                for (let i = 0; i < arrCount; i++) {
                    const item = arr[i];
                    item._renderMode = IgeTextureRenderMode_1.IgeTextureRenderMode.image;
            ...
            if (image._loaded) {
                ...
                this._renderMode = IgeTextureRenderMode_1.IgeTextureRenderMode.image;
    ...
    _loadScript(scriptUrl) {
        var _a;
        if (clientServer_1.isClient) {
            (_a = scriptUrl, Promise.resolve().then(() => __importStar(require(_a)))).then(({ image }) => {
    ...
    assignSmartTextureImage(scriptObj) {
        ...
        this._renderMode = IgeTextureRenderMode_1.IgeTextureRenderMode.smartTexture;
    ...
    render(ctx, entity) {
        ...
        if (instance_1.ige.engine._ctx) {
            instance_1.ige.engine._ctx.imageSmoothingEnabled = this._smoothing;
        }
        if (this._renderMode === IgeTextureRenderMode_1.IgeTextureRenderMode.image) {
        ...
        if (this._renderMode === IgeTextureRenderMode_1.IgeTextureRenderMode.smartTexture) {
            if (!this.script) {
    ...
    destroy() {
        ...
        if (this.image && this.image._igeTextures) {
            (0, utils_1.arrPull)(this.image._igeTextures, this);
        }
