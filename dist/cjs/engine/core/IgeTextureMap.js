...
class IgeTextureMap extends IgeTileMap2d_1.IgeTileMap2d {
	...
	tick(ctx) {
		...
		const mapData = this.map._mapData,
			tileEntity = this._newTileEntity(); // TODO: This is wasteful, cache it?
		let x,
			y,
			tx,
			ty,
						...
						if (mapData.hasOwnProperty(y)) {
							for (x in mapData[y]) {
								if (mapData[y].hasOwnProperty(x)) {
									...
									if (this._mountMode === IgeMountMode_1.IgeMountMode.iso) {
	...
	_drawSectionsToCtx(ctx, autoSection) {
		...
		const viewArea = instance_1.ige.engine._currentViewport.viewArea(),
			sectionWidth = this._tileWidth * autoSection,
			sectionHeight = this._tileHeight * autoSection;
			...
			if (this._sections.hasOwnProperty(x)) {
				for (y in this._sections[x]) {
					if (this._sections[x].hasOwnProperty(y)) {
						...
						sectionAbsX =
							this._translate.x + sectionRenderX - instance_1.ige.engine._currentCamera._translate.x;
						sectionAbsY =
							this._translate.y + sectionRenderY - instance_1.ige.engine._currentCamera._translate.y;
						...
						if (
							sectionAbsX + sectionWidth + this._tileHeight / 2 >= -(viewArea.width / 2) &&
							...
							if (this._drawSectionBounds) {
								...
								ctx.strokeRect(
									x * (this._tileWidth * autoSection),
									y * (this._tileHeight * autoSection),
									this._tileWidth * autoSection,
									this._tileHeight * autoSection
								);
							}
						}
					}
	...
	_renderTile(ctx, x, y, tileData, tileEntity, rect, sectionX, sectionY) {
		...
		const xAdjust = this._mountMode === IgeMountMode_1.IgeMountMode.iso ? this._tileWidth / 2 : 0,
			yAdjust = this._mountMode === IgeMountMode_1.IgeMountMode.iso ? this._tileHeight / 2 : 0;
...
exports.IgeTextureMap = IgeTextureMap;
