...
class IgeTweenController extends IgeEventingClass_1.IgeEventingClass {
					...
					if (tween._started || (tween._startTime !== undefined && currentTime >= tween._startTime)) {
						...
						if (deltaTime >= destTime) {
							...
							for (const targetIndex in targets) {
								if (targets.hasOwnProperty(targetIndex)) {
									...
									if (destTime !== 0) {
										...
										currentDelta = easing_1.easingFunctions[easing](
											destTime,
											item.deltaVal,
											destTime
										);
									...
									const roundingPrecision = Math.pow(
										10,
										15 - targetPropVal.toFixed(0).toString().length
									);
							...
							for (const targetIndex in targets) {
								if (targets.hasOwnProperty(targetIndex)) {
									const item = targets[targetIndex];
									const currentDelta = easing_1.easingFunctions[easing](
										deltaTime,
										item.deltaVal,
										destTime
									);
	...
	stop(tween) {
		...
		(0, utils_1.arrPull)(this._tweens, tween);
