...
class IgeViewport extends IgeUiEntity_1.IgeUiEntity {
	constructor(options) {
		...
		this._bounds2d = new IgePoint2d_1.IgePoint2d(
	...
	tick(ctx) {
		...
		if (this._clipping || this._borderColor) {
			...
			ctx.rect(
				0,
				0,
				this._bounds2d.x / instance_1.ige.engine._scale.x,
				this._bounds2d.y / instance_1.ige.engine._scale.x
			);
		...
		ctx.translate(
			((this._bounds2d.x / 2) | 0) + instance_1.ige.engine._translate.x,
			((this._bounds2d.y / 2) | 0) + instance_1.ige.engine._translate.y
		);
		...
		ctx.restore();
		if (this._drawGuides && ctx === instance_1.ige.engine._ctx) {
			ctx.save();
			ctx.translate(-this._translate.x, -this._translate.y);
			this.paintGuides(ctx);
			ctx.restore();
		}
		if (this._drawBounds && ctx === instance_1.ige.engine._ctx) {
		...
		if (this._drawMouse && ctx === instance_1.ige.engine._ctx) {
			ctx.save();
		...
		return new IgePoint3d_1.IgePoint3d(
			Math.floor(
				this._worldMatrix.matrix[2] +
					(((_a = instance_1.ige.engine._bounds2d) === null || _a === void 0 ? void 0 : _a.x2) || 0)
			),
			Math.floor(
				this._worldMatrix.matrix[5] +
					(((_b = instance_1.ige.engine._bounds2d) === null || _b === void 0 ? void 0 : _b.y2) || 0)
			),
			0
		);
	}
	...
	paintGuides(ctx) {
		if (!instance_1.ige.engine) return;
					...
					if (typeof obj.aabb === "function") {
						...
						if (aabb) {
							if (obj._drawBounds || obj._drawBounds === undefined) {
								...
								if (obj._parent && obj._parent._mountMode === IgeMountMode_1.IgeMountMode.iso) {
