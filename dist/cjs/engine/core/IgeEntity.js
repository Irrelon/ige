...
class IgeEntity extends IgeObject_1.IgeObject {
	constructor() {
		...
		this.mouseEventTrigger = (val) => {
			this.log(
				"mouseEventTrigger is no longer in use. Please see triggerPolygonFunctionName() instead.",
				"warning"
			);
		};
		/**
	...
	updateTransform() {
		...
		if (this._renderMode === IgeEntityRenderMode_1.IgeEntityRenderMode.iso) {
			// iso translation
			const isoPoint = (this._translateIso = new IgePoint3d_1.IgePoint3d(
				this._translate.x,
				this._translate.y,
	...
	rotateToPoint(point) {
		...
		this.rotateTo(
			...
			Math.atan2(worldPos.y - point.y, worldPos.x - point.x) -
				((_c =
					(_b = (_a = this._parent) === null || _a === void 0 ? void 0 : _a._rotate) === null || _b === void 0
						? void 0
						: _b.z) !== null && _c !== void 0
					? _c
					: 0) +
	...
	heightByTile(val, lockAspect = false) {
		...
		const tileSize =
			this._renderMode === IgeEntityRenderMode_1.IgeEntityRenderMode.flat
				? this._parent._tileHeight
				: this._parent._tileHeight * 2;
	...
	occupyTile(x, y, width, height) {
		...
		const trPoint = new IgePoint3d_1.IgePoint3d(
	...
	localAabb(recalculate = false) {
		...
		this._localAabb = new IgeRect_1.IgeRect(
	...
	isBehind(otherObject) {
		...
		const thisX = thisTranslate.x,
			...
			thisMin = new IgePoint3d_1.IgePoint3d(thisX - thisG3d.x / 2, thisY - thisG3d.y / 2, this._translate.z),
			thisMax = new IgePoint3d_1.IgePoint3d(
				thisX + thisG3d.x / 2,
				thisY + thisG3d.y / 2,
				this._translate.z + thisG3d.z
			),
			otherMin = new IgePoint3d_1.IgePoint3d(
				otherX - otherG3d.x / 2,
				otherY - otherG3d.y / 2,
				otherObject._translate.z
			),
			otherMax = new IgePoint3d_1.IgePoint3d(
				otherX + otherG3d.x / 2,
				otherY + otherG3d.y / 2,
	...
	_transformContext(ctx, inverse = false) {
		...
		if (!inverse) {
			...
			(_a = this._localMatrix.getInverse()) === null || _a === void 0
				? void 0
			...
			if (this._backgroundPatternFill) {
				...
				if (this._smartBackground) {
					...
					if (this._backgroundPatternTrackCamera) {
						ctx.translate(
							-instance_1.ige.engine._currentCamera._translate.x,
							-instance_1.ige.engine._currentCamera._translate.y
						);
						ctx.scale(
							instance_1.ige.engine._currentCamera._scale.x,
							instance_1.ige.engine._currentCamera._scale.y
						);
					}
					ctx.fill();
	 ...
	 */
	_renderCache(ctx) {
		if (!instance_1.ige.engine._currentViewport) return;
		if (!this._cacheCanvas || this._cacheCanvas instanceof IgeDummyCanvas_1.IgeDummyCanvas) return;
		ctx.save();
	...
	isometric(val) {
		if (val !== undefined) {
			...
			this._renderMode = val
				? IgeEntityRenderMode_1.IgeEntityRenderMode.iso
				: IgeEntityRenderMode_1.IgeEntityRenderMode.flat;
			return this;
		}
