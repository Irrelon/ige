	...
	constructor() {
		...
		this.fireManualEvent = (eventName, eventObj) => {
			if (eventName && eventObj) {
				...
				this.log(
					"Cannot fire manual event because both eventName and eventObj params are required.",
					"warning"
				);
			}
		};
		...
		this._pointerDown = (event) => {
			...
			if (event.button === 0) {
				this._updateState(
					IgeInputDeviceMap_1.IgeInputDevice.pointer1,
					IgeInputDeviceMap_1.IgeInputPointerMap.button0,
					true
				);
			}
			if (event.button === 1) {
				this._updateState(
					IgeInputDeviceMap_1.IgeInputDevice.pointer1,
					IgeInputDeviceMap_1.IgeInputPointerMap.button1,
					true
				);
			}
			if (event.button === 2) {
				this._updateState(
					IgeInputDeviceMap_1.IgeInputDevice.pointer1,
					IgeInputDeviceMap_1.IgeInputPointerMap.button2,
					true
				);
			}
			if (event.button === 3) {
				this._updateState(
					IgeInputDeviceMap_1.IgeInputDevice.pointer1,
					IgeInputDeviceMap_1.IgeInputPointerMap.button3,
					true
				);
			}
			if (event.button === 4) {
				this._updateState(
					IgeInputDeviceMap_1.IgeInputDevice.pointer1,
					IgeInputDeviceMap_1.IgeInputPointerMap.button4,
					true
				);
			}
			if (event.button === 5) {
				this._updateState(
					IgeInputDeviceMap_1.IgeInputDevice.pointer1,
					IgeInputDeviceMap_1.IgeInputPointerMap.button5,
					true
				);
			}
			this.pointerDown = event;
			if (
				this.emit("prePointerDown", event, mx, my, event.button) !==
				IgeEventReturnFlag_1.IgeEventReturnFlag.cancel
			) {
				this.queueEvent(() => {
		...
		this._pointerUp = (event) => {
			...
			if (
				this.emit("prePointerUp", event, mx, my, event.button) !==
				IgeEventReturnFlag_1.IgeEventReturnFlag.cancel
			) {
				this.queueEvent(() => {
		...
		this._contextMenu = (event) => {
			...
			if (
				this.emit("preContextMenu", event, mx, my, event.button) !==
				IgeEventReturnFlag_1.IgeEventReturnFlag.cancel
			) {
				this.queueEvent(() => {
		...
		this._pointerMove = (event) => {
			...
			this._updateState(
				IgeInputDeviceMap_1.IgeInputDevice.pointer1,
				IgeInputDeviceMap_1.IgeInputPointerMap.x,
				mx
			);
			this._updateState(
				IgeInputDeviceMap_1.IgeInputDevice.pointer1,
				IgeInputDeviceMap_1.IgeInputPointerMap.y,
				my
			);
			this.pointerMove = event;
			if (
				this.emit("prePointerMove", event, mx, my, event.button) !==
				IgeEventReturnFlag_1.IgeEventReturnFlag.cancel
			) {
				this.queueEvent(() => {
		...
		this._pointerWheel = (event) => {
			...
			if (event.deltaX !== 0) {
				this._updateState(
					IgeInputDeviceMap_1.IgeInputDevice.pointer1,
					IgeInputDeviceMap_1.IgeInputPointerMap.wheelX,
					event.deltaX
				);
				if (event.deltaX > 0) {
					this._updateState(
						IgeInputDeviceMap_1.IgeInputDevice.pointer1,
						IgeInputDeviceMap_1.IgeInputPointerMap.wheelRight,
						true
					);
					this._updateState(
						IgeInputDeviceMap_1.IgeInputDevice.pointer1,
						IgeInputDeviceMap_1.IgeInputPointerMap.wheelLeft,
						false
					);
				} else {
					this._updateState(
			...
			if (event.deltaY !== 0) {
				this._updateState(
					IgeInputDeviceMap_1.IgeInputDevice.pointer1,
					IgeInputDeviceMap_1.IgeInputPointerMap.wheelY,
					event.deltaY
				);
				if (event.deltaY > 0) {
					this._updateState(
						IgeInputDeviceMap_1.IgeInputDevice.pointer1,
						IgeInputDeviceMap_1.IgeInputPointerMap.wheelUp,
						true
					);
					this._updateState(
						IgeInputDeviceMap_1.IgeInputDevice.pointer1,
						IgeInputDeviceMap_1.IgeInputPointerMap.wheelDown,
						false
					);
				} else {
					this._updateState(
			...
			if (event.deltaZ !== 0) {
				this._updateState(
					IgeInputDeviceMap_1.IgeInputDevice.pointer1,
					IgeInputDeviceMap_1.IgeInputPointerMap.wheelZ,
					event.deltaZ
				);
				if (event.deltaZ > 0) {
					this._updateState(
						IgeInputDeviceMap_1.IgeInputDevice.pointer1,
						IgeInputDeviceMap_1.IgeInputPointerMap.wheelBackward,
						true
					);
					this._updateState(
						IgeInputDeviceMap_1.IgeInputDevice.pointer1,
						IgeInputDeviceMap_1.IgeInputPointerMap.wheelForward,
						false
					);
				} else {
					this._updateState(
			...
			if (
				this.emit("prePointerWheel", event, mx, my, event.button) !==
				IgeEventReturnFlag_1.IgeEventReturnFlag.cancel
			) {
				this.queueEvent(() => {
		...
		this._keyDown = (event) => {
			this._updateState(
				IgeInputDeviceMap_1.IgeInputDevice.keyboard,
				IgeInputDeviceMap_1.IgeInputKeyboardMap[event.code],
				true
			);
			if (this._debug) {
			...
			while (arrCount--) {
				...
				if (mx > vp._translate.x - vp._bounds2d.x / 2 && mx < vp._translate.x + vp._bounds2d.x / 2) {
					if (my > vp._translate.y - vp._bounds2d.y / 2 && my < vp._translate.y + vp._bounds2d.y / 2) {
						...
						vp._pointerPos = new IgePoint3d_1.IgePoint3d(
		...
		return new Promise((resolve) => {
			setTimeout(() => {
				instance_1.ige.dependencies.waitFor(["engine"], () => {
					...
					instance_1.ige.engine.addBehaviour(
						IgeBehaviourType_1.IgeBehaviourType.postTick,
						"inputComponentPostTick",
						this.tick.bind(this)
					);
					resolve();
				});
