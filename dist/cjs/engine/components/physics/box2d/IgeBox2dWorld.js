...
class IgeBox2dWorld extends IgeEventingClass_1.IgeEventingClass {
    constructor(entity, options) {
        ...
        this.b2Distance = lib_box2d_1.Box2D.Collision.b2Distance;
    ...
    enableDebug(mountScene) {
        if (mountScene) {
            ...
            debugDraw.SetSprite(instance_1.ige._ctx);
            ...
            new igeClassStore_1.igeClassStore.IgeBox2dDebugPainter()
        ...
        if (!this._active) {
            ...
            if (!this._networkDebugMode) {
                if (this._renderMode === IgeBox2dTimingMode_1.IgeBox2dTimingMode.matchEngine) {
                    ...
                    instance_1.ige.engine.addBehaviour(IgeBehaviourType_1.IgeBehaviourType.preUpdate, `box2dStep_${this._id}`, this._behaviour);
                }
                else {
    ...
    stop() {
        if (this._active) {
            ...
            if (this._renderMode === IgeBox2dTimingMode_1.IgeBox2dTimingMode.matchEngine) {
                // Add the box2d behaviour to the ige
                instance_1.ige.engine.removeBehaviour(IgeBehaviourType_1.IgeBehaviourType.preUpdate, 'box2dStep_' + this._id);
            }
            else {
    ...
    _behaviour(entity, ctx) {
        ...
        if (self._active && self._world) {
            ...
            if (self._renderMode === IgeBox2dTimingMode_1.IgeBox2dTimingMode.matchEngine) {
                self._world.Step(instance_1.ige._tickDelta / 1000, 8, 3);
            }
            else {
...
exports.IgeBox2dWorld = IgeBox2dWorld;
