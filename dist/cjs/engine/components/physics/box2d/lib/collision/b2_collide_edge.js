...
const b2_collision_2 = require("./b2_collision");
...
function b2CollideEdgeAndCircle(manifold, edgeA, xfA, circleB, xfB) {
    ...
    const Q = b2_math_1.b2Transform.MulTXV(xfA, b2_math_1.b2Transform.MulXV(xfB, circleB.m_p, b2_math_1.b2Vec2.s_t0), b2CollideEdgeAndCircle_s_Q);
    const A = edgeA.m_vertex1;
    const B = edgeA.m_vertex2;
    const e = b2_math_1.b2Vec2.SubVV(B, A, b2CollideEdgeAndCircle_s_e);
    ...
    const offset = b2_math_1.b2Vec2.DotVV(n, b2_math_1.b2Vec2.SubVV(Q, A, b2_math_1.b2Vec2.s_t0));
    const oneSided = edgeA.m_oneSided;
    if (oneSided && offset < 0.0) {
        return;
    }
    // Barycentric coordinates
    const u = b2_math_1.b2Vec2.DotVV(e, b2_math_1.b2Vec2.SubVV(B, Q, b2_math_1.b2Vec2.s_t0));
    const v = b2_math_1.b2Vec2.DotVV(e, b2_math_1.b2Vec2.SubVV(Q, A, b2_math_1.b2Vec2.s_t0));
    ...
    if (v <= 0) {
        const P = A;
        const d = b2_math_1.b2Vec2.SubVV(Q, P, b2CollideEdgeAndCircle_s_d);
        const dd = b2_math_1.b2Vec2.DotVV(d, d);
        if (dd > radius * radius) {
            return;
        }
        ...
        if (edgeA.m_oneSided) {
            ...
            const B1 = A;
            const e1 = b2_math_1.b2Vec2.SubVV(B1, A1, b2CollideEdgeAndCircle_s_e1);
            const u1 = b2_math_1.b2Vec2.DotVV(e1, b2_math_1.b2Vec2.SubVV(B1, Q, b2_math_1.b2Vec2.s_t0));
    ...
    if (u <= 0) {
        ...
        if (edgeA.m_oneSided) {
            ...
            const A2 = B;
            const e2 = b2_math_1.b2Vec2.SubVV(B2, A2, b2CollideEdgeAndCircle_s_e2);
            const v2 = b2_math_1.b2Vec2.DotVV(e2, b2_math_1.b2Vec2.SubVV(Q, A2, b2_math_1.b2Vec2.s_t0));
    ...
    // Region AB
    const den = b2_math_1.b2Vec2.DotVV(e, e);
...
function b2ComputeEdgeSeparation(polygonB, v1, normal1) {
    ...
    for (let j = 0; j < 2; ++j) {
        ...
        for (let i = 0; i < polygonB.count; ++i) {
            ...
            const si = b2_math_1.b2Vec2.DotVV(axes[j], b2_math_1.b2Vec2.SubVV(polygonB.vertices[i], v1, b2_math_1.b2Vec2.s_t0));
            if (si < sj) {
                sj = si;
            }
        }
...
function b2ComputePolygonSeparation(polygonB, v1, v2) {
    ...
    for (let i = 0; i < polygonB.count; ++i) {
        ...
        const n = b2_math_1.b2Vec2.NegV(polygonB.normals[i], b2ComputePolygonSeparation_s_n);
        // float s1 = b2Dot(n, polygonB.vertices[i] - v1);
        const s1 = b2_math_1.b2Vec2.DotVV(n, b2_math_1.b2Vec2.SubVV(polygonB.vertices[i], v1, b2_math_1.b2Vec2.s_t0));
        // float s2 = b2Dot(n, polygonB.vertices[i] - v2);
        const s2 = b2_math_1.b2Vec2.DotVV(n, b2_math_1.b2Vec2.SubVV(polygonB.vertices[i], v2, b2_math_1.b2Vec2.s_t0));
        // float s = b2Min(s1, s2);
        const s = (0, b2_math_1.b2Min)(s1, s2);
...
const b2CollideEdgeAndPolygon_s_clipPoints = [new b2_collision_2.b2ClipVertex(), new b2_collision_2.b2ClipVertex()];
const b2CollideEdgeAndPolygon_s_clipPoints1 = [new b2_collision_2.b2ClipVertex(), new b2_collision_2.b2ClipVertex()];
const b2CollideEdgeAndPolygon_s_clipPoints2 = [new b2_collision_2.b2ClipVertex(), new b2_collision_2.b2ClipVertex()];
function b2CollideEdgeAndPolygon(manifold, edgeA, xfA, polygonB, xfB) {
    manifold.pointCount = 0;
    // b2Transform xf = b2MulT(xfA, xfB);
    const xf = b2_math_1.b2Transform.MulTXX(xfA, xfB, b2CollideEdgeAndPolygon_s_xf);
    // b2Vec2 centroidB = b2Mul(xf, polygonB.m_centroid);
    const centroidB = b2_math_1.b2Transform.MulXV(xf, polygonB.m_centroid, b2CollideEdgeAndPolygon_s_centroidB);
    // b2Vec2 v1 = edgeA.m_vertex1;
    const v1 = edgeA.m_vertex1;
    // b2Vec2 v2 = edgeA.m_vertex2;
    const v2 = edgeA.m_vertex2;
    // b2Vec2 edge1 = v2 - v1;
    const edge1 = b2_math_1.b2Vec2.SubVV(v2, v1, b2CollideEdgeAndPolygon_s_edge1);
    edge1.Normalize();
    // Normal points to the right for a CCW winding
    ...
    const offset1 = b2_math_1.b2Vec2.DotVV(normal1, b2_math_1.b2Vec2.SubVV(centroidB, v1, b2_math_1.b2Vec2.s_t0));
    const oneSided = edgeA.m_oneSided;
    ...
    if (oneSided) {
        ...
        const edge0 = b2_math_1.b2Vec2.SubVV(v1, edgeA.m_vertex0, b2CollideEdgeAndPolygon_s_edge0);
        edge0.Normalize();
        // b2Vec2 normal0(edge0.y, -edge0.x);
        const normal0 = b2CollideEdgeAndPolygon_s_normal0.Set(edge0.y, -edge0.x);
        const convex1 = b2_math_1.b2Vec2.CrossVV(edge0, edge1) >= 0.0;
        // b2Vec2 edge2 = edgeA.m_vertex3 - v2;
        const edge2 = b2_math_1.b2Vec2.SubVV(edgeA.m_vertex3, v2, b2CollideEdgeAndPolygon_s_edge2);
        edge2.Normalize();
        // b2Vec2 normal2(edge2.y, -edge2.x);
        const normal2 = b2CollideEdgeAndPolygon_s_normal2.Set(edge2.y, -edge2.x);
        const convex2 = b2_math_1.b2Vec2.CrossVV(edge1, edge2) >= 0.0;
        const sinTol = 0.1;
        const side1 = b2_math_1.b2Vec2.DotVV(primaryAxis.normal, edge1) <= 0.0;
        // Check Gauss Map
        if (side1) {
            if (convex1) {
    ...
    if (primaryAxis.type === b2EPAxisType.e_edgeA) {
        ...
        let bestIndex = 0;
        let bestValue = b2_math_1.b2Vec2.DotVV(primaryAxis.normal, tempPolygonB.normals[0]);
        for (let i = 1; i < tempPolygonB.count; ++i) {
            const value = b2_math_1.b2Vec2.DotVV(primaryAxis.normal, tempPolygonB.normals[i]);
            if (value < bestValue) {
                bestValue = value;
                bestIndex = i;
            }
    ...
    let np;
    // Clip to side 1
    np = (0, b2_collision_2.b2ClipSegmentToLine)(clipPoints1, clipPoints, ref.sideNormal1, ref.sideOffset1, ref.i1);
    if (np < b2_settings_1.b2_maxManifoldPoints) {
        return;
    }
    // Clip to side 2
    np = (0, b2_collision_2.b2ClipSegmentToLine)(clipPoints2, clipPoints1, ref.sideNormal2, ref.sideOffset2, ref.i2);
    if (np < b2_settings_1.b2_maxManifoldPoints) {
        return;
    }
    ...
    for (let i = 0; i < b2_settings_1.b2_maxManifoldPoints; ++i) {
        const separation = b2_math_1.b2Vec2.DotVV(ref.normal, b2_math_1.b2Vec2.SubVV(clipPoints2[i].v, ref.v1, b2_math_1.b2Vec2.s_t0));
        if (separation <= radius) {
            const cp = manifold.points[pointCount];
            if (primaryAxis.type === b2EPAxisType.e_edgeA) {
                b2_math_1.b2Transform.MulTXV(xf, clipPoints2[i].v, cp.localPoint); // cp.localPoint = b2MulT(xf, clipPoints2[i].v);
