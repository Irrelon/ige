...
const lib_box2d_1 = require("./lib_box2d.js");
...
class IgeBox2dController extends IgeEventingClass_1.IgeEventingClass {
	constructor() {
		...
		this._active = false;
		this._renderMode = IgeBox2dTimingMode_1.IgeBox2dTimingMode.matchEngine;
		this._useWorker = false;
		this._sleep = true;
		this._scaleRatio = 1;
		this._networkDebugMode = false;
		this._box2dDebug = false;
		this.b2Color = lib_box2d_1.Box2D.Common.b2Color;
		this.b2Vec2 = lib_box2d_1.Box2D.Common.Math.b2Vec2;
		this.b2Math = lib_box2d_1.Box2D.Common.Math.b2Math;
		this.b2Shape = lib_box2d_1.Box2D.Collision.Shapes.b2Shape;
		this.b2BodyDef = lib_box2d_1.Box2D.Dynamics.b2BodyDef;
		this.b2Body = lib_box2d_1.Box2D.Dynamics.b2Body;
		this.b2FixtureDef = lib_box2d_1.Box2D.Dynamics.b2FixtureDef;
		this.b2Fixture = lib_box2d_1.Box2D.Dynamics.b2Fixture;
		this.b2World = lib_box2d_1.Box2D.Dynamics.b2World;
		this.b2MassData = lib_box2d_1.Box2D.Collision.Shapes.b2MassData;
		this.b2PolygonShape = lib_box2d_1.Box2D.Collision.Shapes.b2PolygonShape;
		this.b2CircleShape = lib_box2d_1.Box2D.Collision.Shapes.b2CircleShape;
		this.b2DebugDraw = lib_box2d_1.Box2D.Dynamics.b2DebugDraw;
		this.b2ContactListener = lib_box2d_1.Box2D.Dynamics.b2ContactListener;
		this.b2Distance = lib_box2d_1.Box2D.Collision.b2DistanceOutput;
		this.b2Contact = lib_box2d_1.Box2D.Dynamics.Contacts.b2Contact;
		this.b2FilterData = lib_box2d_1.Box2D.Dynamics.b2FilterData;
		this.b2DistanceJointDef = lib_box2d_1.Box2D.Dynamics.Joints.b2DistanceJointDef;
		/**
		...
		this._behaviour = () => {
			...
			if (this._active && this._world) {
				...
				if (this._renderMode === IgeBox2dTimingMode_1.IgeBox2dTimingMode.matchEngine) {
				...
				while (tempBod) {
					if (tempBod._entity) {
						...
						if (tempBod.IsAwake() && tempBod.m_type !== 0) {
							...
							entity.translateTo(
								tempBod.m_xf.position.x * this._scaleRatio,
								tempBod.m_xf.position.y * this._scaleRatio,
								entity._translate.z
							);
	...
	start() {
		if (!this._world) {
			throw new Error(
				"Cannot start the physics simulation until a world exists. Use the createWorld() method to set up a physics world before calling start()!"
			);
		}
		if (this._active) {
			return;
		...
		if (this._renderMode === IgeBox2dTimingMode_1.IgeBox2dTimingMode.matchEngine) {
			instance_1.ige.engine.addBehaviour(
				IgeBehaviourType_1.IgeBehaviourType.preUpdate,
				"box2dStep",
				this._behaviour
			);
		} else {
	...
	stop() {
		...
		if (this._renderMode === IgeBox2dTimingMode_1.IgeBox2dTimingMode.matchEngine) {
			instance_1.ige.engine.removeBehaviour(IgeBehaviourType_1.IgeBehaviourType.preUpdate, "box2dStep");
		} else {
	...
	createBody(entity, body) {
		...
		switch (body.type) {
			case IgeBox2dBodyType_1.IgeBox2dBodyType.static: // "static"
				tempDef.type = this.b2Body.b2_staticBody;
				break;
			case IgeBox2dBodyType_1.IgeBox2dBodyType.dynamic: // "dynamic"
				tempDef.type = this.b2Body.b2_dynamicBody;
				break;
			case IgeBox2dBodyType_1.IgeBox2dBodyType.kinematic: // "kinematic"
						...
						for (i = 0; i < body.fixtures.length; i++) {
							...
							if (fixtureDef.shape) {
								...
								switch (fixtureDef.shape.type) {
									case IgeBox2dFixtureShapeType_1.IgeBox2dFixtureShapeType.circle:
										...
										break;
									case IgeBox2dFixtureShapeType_1.IgeBox2dFixtureShapeType.polygon:
										...
										break;
									case IgeBox2dFixtureShapeType_1.IgeBox2dFixtureShapeType.rectangle:
										tempShape = new this.b2PolygonShape();
										if (fixtureDef.shape.data) {
											finalX =
												fixtureDef.shape.data.x !== undefined ? fixtureDef.shape.data.x : 0;
	...
	enableDebug(mountScene) {
		...
		if (mountScene) {
			...
			debugDraw.SetSprite(instance_1.ige.engine._ctx);
			...
			new IgeBox2dDebugPainter_1.IgeBox2dDebugPainter(instance_1.ige.engine)
...
exports.IgeBox2dController = IgeBox2dController;
