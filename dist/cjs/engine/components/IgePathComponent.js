	...
	constructor(entity, options) {
		...
		this.set = (fromX, fromY, fromZ, toX, toY, toZ, findNearest = false) => {
			...
			const path = this._finder.generate(
				this._tileMap,
				new IgePathNode_1.IgePathNode(fromX, fromY, fromZ),
				new IgePathNode_1.IgePathNode(toX, toY, toZ),
				this._tileChecker,
				this._allowSquare,
				this._allowDiagonal,
				findNearest
		...
		this.add = (x, y, z, findNearest = false) => {
			...
			const path = this._finder.generate(
				this._tileMap,
				endPoint,
				new IgePathNode_1.IgePathNode(x, y, z),
				this._tileChecker,
				this._allowSquare,
				this._allowDiagonal,
				findNearest
		...
		this._processDynamic = (pointFrom, pointTo, destinationPoint) => {
			...
			if (!this._tileChecker(tileCheckData, pointTo.x, pointTo.y, null, null, null, true)) {
				...
				newPathPoints = this._finder.generate(
					this._tileMap,
					new IgePathNode_1.IgePathNode(pointFrom.x, pointFrom.y, pointFrom.z),
					new IgePathNode_1.IgePathNode(destinationPoint.x, destinationPoint.y, destinationPoint.z),
					this._tileChecker,
					this._allowSquare,
					this._allowDiagonal,
					false
				...
				if (newPathPoints.length) {
					this.replacePoints(
						this._currentPointFrom,
						this._points.length - this._currentPointFrom,
						newPathPoints
					);
					return true;
				} else {
					...
					this.emit(
						"dynamicFail",
						this._entity,
						new IgePoint3d_1.IgePoint3d(pointFrom.x, pointFrom.y, pointFrom.z),
						new IgePoint3d_1.IgePoint3d(destinationPoint.x, destinationPoint.y, destinationPoint.z)
					);
					this.clear();
					return -1;
				}
		...
		this._tickBehaviour = (entity, ctx) => {
			if (!clientServer_1.isClient) return;
			if (!this._tileMap) throw new Error("No tile map (IgeTileMap2d) assigned to IgePathComponent");
			...
			if (currentPath.length) {
				if (currentPath && this._drawPath) {
					...
					for (pathPointIndex = 0; pathPointIndex < currentPath.length; pathPointIndex++) {
						...
						tracePathPoint = new IgePoint3d_1.IgePoint3d(
		...
		this._positionAlongVector = (p1, p2, speed, deltaTime) => {
			...
			if (deltaX !== 0 || deltaY !== 0) {
				newPoint = new IgePoint3d_1.IgePoint3d(
