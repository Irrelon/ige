	...
	constructor(entity, options) {
		...
		this.areaCenter = (x, y) => {
			if (x !== undefined && y !== undefined) {
				...
				let offset;
				if (ent._renderMode === IgeEntityRenderMode_1.IgeEntityRenderMode.flat) {
					// 2d mode
					offset = ent._translate;
				}
				if (ent._renderMode === IgeEntityRenderMode_1.IgeEntityRenderMode.iso) {
					// Iso mode
		...
		this._behaviour = (entity, ctx) => {
			...
			if (self._areaCenter && self._areaRect && !currentArea.compare(self._lastArea)) {
				...
				if (this._entity._mountMode === IgeMountMode_1.IgeMountMode.flat) {
					// 2d
					currentAreaTiles = new IgeRect_1.IgeRect(
				...
				if (this._entity._mountMode === IgeMountMode_1.IgeMountMode.iso) {
					// Isometric
		...
		this._createArr = [];
		this._removeArr = [];
		entity.addBehaviour(IgeBehaviourType_1.IgeBehaviourType.preUpdate, "entityManager", this._behaviour);
	}
}
