 ...
 */
class IgeEditorComponent extends IgeComponent_1.IgeComponent {
    /**
     * @constructor
    ...
    constructor(entity, options) {
        ...
        this.toggle = () => {
            ...
            if (elem.hasClass("active")) {
                instance_1.ige.components.editor.hide();
            }
            else {
                instance_1.ige.components.editor.show();
            }
        };
        this.show = () => {
            this.enabled(true);
            this._show = true;
            $("#editorToggle").html("Editor On").removeClass("active").addClass("active");
            $(".editorElem.toggleHide").addClass("shown");
        };
        this.hide = () => {
            this.enabled(false);
            this._show = false;
            $("#editorToggle").html("Editor Off").removeClass("active");
            $(".editorElem.toggleHide").removeClass("shown");
        };
        this.toggleStats = () => {
            const elem = $("#statsToggle");
            if (elem.hasClass("active")) {
                instance_1.ige.components.editor.hideStats();
            }
            else {
                instance_1.ige.components.editor.showStats();
            }
        };
        this.showStats = () => {
            $("#statsToggle").html("Stats On").removeClass("active").addClass("active");
            $(".counter").show();
        };
        this.hideStats = () => {
            $("#statsToggle").html("Stats Off").removeClass("active");
            $(".counter").hide();
        };
        ...
        this.selectObject = (id) => {
            if (id !== undefined) {
                if (id) {
                    this._selectedObject = instance_1.ige.$(id);
        ...
        this._objectSelected = (obj) => {
            if (obj) {
                instance_1.ige.components.editor.ui.panels.showPanelByInstance(obj);
                this._selectedObjectClassList = instance_1.ige.getClassDerivedList(obj);
                // Update active-for selectors
                $("[data-active-for]").removeClass("disabled").addClass("disabled");
                let classArr = this._selectedObjectClassList, i;
                for (i = 0; i < classArr.length; i++) {
                    $('[data-active-for~="' + classArr[i] + '"]').removeClass("disabled");
                }
        ...
        this.createObject = (classId, select) => {
            if (this._selectedObject) {
                const newObj = instance_1.ige.newClassInstance(classId);
         ...
         */
        this._statsTick = () => {
            let self = instance_1.ige.components.editor, i, watchCount, watchItem, itemName, res, html = "";
        ...
        this.addToSgTree = (item) => {
            let elem = document.createElement("li"), arr, arrCount, i, pointerUp, dblClick, timingString;
            pointerUp = function (event) {
                ...
                instance_1.ige._sgTreeSelected = this.id;
                instance_1.ige._currentViewport.drawBounds(true);
                if (this.id !== "ige") {
                    instance_1.ige._currentViewport.drawBoundsLimitId(this.id);
                }
                else {
                    instance_1.ige._currentViewport.drawBoundsLimitId("");
                }
                instance_1.ige.emit("sgTreeSelectionChanged", instance_1.ige._sgTreeSelected);
            };
            ...
            if (instance_1.ige._sgTreeSelected === item.id) {
                elem.className += " selected";
            }
            if (igeConfig.debug._timing) {
                if (instance_1.ige._timeSpentInTick[item.id]) {
                    timingString = "<span>" + instance_1.ige._timeSpentInTick[item.id] + "ms</span>";
            ...
            if (item.items) {
                ...
                for (i = 0; i < arrCount; i++) {
                    instance_1.ige.addToSgTree(arr[i]);
                }
            }
        };
        this.toggleShowEditor = () => {
            ...
            if (this._showSgTree) {
                ...
                canvasBoundingRect = instance_1.ige._canvasPosition();
                elem1.id = "igeSgTree";
                elem1.style.top = parseInt(canvasBoundingRect.top) + 5 + "px";
                elem1.style.left = parseInt(canvasBoundingRect.left) + 5 + "px";
                elem1.style.height = instance_1.ige.engine._bounds2d.y - 30 + "px";
                ...
                consoleHolderElem.innerHTML =
                    "<div><b>Console</b>: Double-Click a SceneGraph Object to Script it Here</div>";
                ...
                editorModeTranslate.addEventListener("click", () => {
                    // Disable other modes
                    instance_1.ige.editorRotate.enabled(false);
                    if (instance_1.ige.editorTranslate.enabled()) {
                        instance_1.ige.editorTranslate.enabled(false);
                        self.log("Editor: Translate mode disabled");
                    }
                    else {
                        instance_1.ige.editorTranslate.enabled(true);
                ...
                editorModeRotate.addEventListener("click", () => {
                    // Disable other modes
                    instance_1.ige.editorTranslate.enabled(false);
                    if (instance_1.ige.editorRotate.enabled()) {
                        instance_1.ige.editorRotate.enabled(false);
                        self.log("Editor: Rotate mode disabled");
                    }
                    else {
                        instance_1.ige.editorRotate.enabled(true);
                ...
                instance_1.ige.addComponent(IgeEditorTranslateComponent);
                instance_1.ige.addComponent(IgeEditorRotateComponent);
                // Schedule tree updates every second
                instance_1.ige._sgTreeUpdateInterval = setInterval(() => {
                    self.sgTreeUpdate();
                }, 1000);
            }
            else {
                // Kill interval
                clearInterval(instance_1.ige._sgTreeUpdateInterval);
                ...
                child.parentNode.removeChild(child);
                instance_1.ige.removeComponent("editorTranslate");
                instance_1.ige.removeComponent("editorRotate");
            }
        };
        this.sgTreeUpdate = () => {
        ...
        this._activateKeyHandle = instance_1.ige.input.on("keyUp", (event) => {
        ...
        this._pointerUpHandle = instance_1.ige.input.on("preMouseUp", (event) => {
        ...
        this._pointerDownHandle = instance_1.ige.input.on("preMouseDown", (event) => {
            if (this._enabled && this._interceptMouse) {
        ...
        this._pointerMoveHandle = instance_1.ige.input.on("preMouseMove", (event) => {
            if (this._enabled && this._interceptMouse) {
        ...
        this._contextMenuHandle = instance_1.ige.input.on("preContextMenu", (event) => {
            if (this._enabled && this._interceptMouse) {
        ...
        instance_1.ige.sync(instance_1.ige.requireScript, [
            igeRoot + "engine/components/editor/react/build/static/js/main.min.js",
            undefined
        ]);
        instance_1.ige.sync(instance_1.ige.requireStylesheet, [igeRoot + "engine/components/editor/react/build/static/css/main.min.css"]);
        instance_1.ige.sync(instance_1.ige.requireStylesheet, [igeRoot + "engine/components/editor/css/editor.css"]);
        instance_1.ige.on("syncComplete", () => {
...
exports.default = IgeEditorComponent;
