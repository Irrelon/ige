...
exports.UiAnimationEditor = void 0;
const IgeEventingClass_1 = require("../../../../core/IgeEventingClass.js");
const instance_1 = require("../../../../instance.js");
class UiAnimationEditor extends IgeEventingClass_1.IgeEventingClass {
    constructor() {
        super(...arguments);
        this.classId = "UiAnimationEditor";
    }
    init() {
        instance_1.ige.engine.requireStylesheet(igeRoot + "components/editor/ui/animationEditor/animationEditor.css");
        this.reset();
    }
    reset() {
        const self = this;
    ...
    ready() {
        const self = this;
        instance_1.ige.editor.ui.menus.addMenuGroup("toolsMenu", "textures");
        instance_1.ige.editor.ui.menus.addMenuItem("toolsMenu", "textures", {
            id: "animationEditor",
            icon: "none",
    ...
    show(settings) {
        ...
        instance_1.ige.editor.ui.dialogs.create({
            ...
            blur() {
                instance_1.ige.editor.ui.dialogs.confirm({
                    ...
                    positive() {
                        self.destroy();
                        instance_1.ige.editor.ui.dialogs.close("animationEditorDialog");
                    }
                });
            },
            width: 1000,
    ...
    setupListeners(dialogElem) {
        ...
        self._framesCanvas.on("click", function (e) {
            const oe = e.originalEvent, cell = self.cellFromXY(oe);
        ...
        self._cellsCanvas.on("click", function (e) {
            let oe = e.originalEvent, cell = self.cellFromXY(oe), frameIndex;
    ...
    cellFromIndex(index) {
        index = index - 1;
        const self = this, cellsPerRow = self._textureImage.width / self._cellWidth, y = Math.floor(index / cellsPerRow), x = index - y * cellsPerRow;
        return { x: x, y: y };
    }
    setupCanvas() {
        const self = this;
    ...
    _renderCanvas(noGrid) {
        let self = this, framesCtx = self._framesCtx, cellsCtx = self._cellsCtx, outputCtx = self._outputCtx, cell, cellWidth, cellHeight, x, y, i, cellIndex;
        ...
        for (i = 0; i < self._frames.length; i++) {
            cell = self._frames[i];
            framesCtx.drawImage(self._textureImage, cell.x * self._cellWidth, cell.y * self._cellHeight, self._cellWidth, self._cellHeight, i * self._cellWidth, 0, self._cellWidth, self._cellHeight);
        }
        // Loop the available cells and draw them
        if (self._textureImage) {
            for (y = 0; y < self._textureImage.height; y += self._cellHeight) {
                for (x = 0; x < self._textureImage.width; x += self._cellWidth) {
                    cellsCtx.drawImage(self._textureImage, parseInt(x) * self._cellWidth, parseInt(y) * self._cellHeight);
                }
            }
        }
        ...
        if (self._animationEntity && self._animationEntity.animation.defined("editorAnim")) {
            ...
            outputCtx.save();
            outputCtx.translate(Math.floor(self._outputCanvas[0].width / 2 - self._cellWidth / 2), Math.floor(self._outputCanvas[0].height / 2 - self._cellHeight / 2));
            outputCtx.drawImage(self._textureImage, cell.x * self._cellWidth, cell.y * self._cellHeight, self._cellWidth, self._cellHeight, 0, 0, self._cellWidth, self._cellHeight);
            outputCtx.restore();
        }
        if (!noGrid) {
...
exports.UiAnimationEditor = UiAnimationEditor;
// Init
