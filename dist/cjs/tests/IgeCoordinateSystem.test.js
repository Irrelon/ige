...
describe("Coordinate System", () => {
    describe("lookAt basis vectors (right-handed)", () => {
        it("produces correct basis vectors for camera on +Z axis", () => {
            ...
            m.lookAt(new IgePoint3d_1.IgePoint3d(0, 0, 5), new IgePoint3d_1.IgePoint3d(0, 0, 0), new IgePoint3d_1.IgePoint3d(0, 1, 0));
        ...
        it("preserves right-handedness for off-axis camera", () => {
            const m = new IgeMatrix4_1.IgeMatrix4();
            m.lookAt(new IgePoint3d_1.IgePoint3d(10, 10, 10), new IgePoint3d_1.IgePoint3d(0, 0, 0), new IgePoint3d_1.IgePoint3d(0, 1, 0));
            // Extract basis vectors from view matrix rows
            const right = new IgePoint3d_1.IgePoint3d(m.matrix[0], m.matrix[4], m.matrix[8]);
            const up = new IgePoint3d_1.IgePoint3d(m.matrix[1], m.matrix[5], m.matrix[9]);
            const forward = new IgePoint3d_1.IgePoint3d(m.matrix[2], m.matrix[6], m.matrix[10]);
            // Right-hand rule: right × up should equal forward
            // (cross product of rows 0 and 1 should give row 2)
            const cross = new IgePoint3d_1.IgePoint3d(right.y * up.z - right.z * up.y, right.z * up.x - right.x * up.z, right.x * up.y - right.y * up.x);
    ...
    describe("orthographic projection mapping", () => {
        it("maps +X world to +X NDC", () => {
            const proj = new IgeMatrix4_1.IgeMatrix4();
            proj.orthographic(-100, 100, -100, 100, -100, 100);
            const point = new IgePoint3d_1.IgePoint3d(50, 0, 0);
        ...
        it("maps +Y world to +Y NDC", () => {
            const proj = new IgeMatrix4_1.IgeMatrix4();
            proj.orthographic(-100, 100, -100, 100, -100, 100);
            const point = new IgePoint3d_1.IgePoint3d(0, 50, 0);
            proj.transformPoint(point);
        ...
        it("maps corners correctly", () => {
            ...
            // Left edge -> NDC -1
            const left = new IgePoint3d_1.IgePoint3d(-10, 0, 0);
            proj.transformPoint(left);
            expect(left.x).toBeCloseTo(-1, 5);
            // Right edge -> NDC +1
            const right = new IgePoint3d_1.IgePoint3d(10, 0, 0);
            proj.transformPoint(right);
            expect(right.x).toBeCloseTo(1, 5);
            // Bottom edge -> NDC -1
            const bottom = new IgePoint3d_1.IgePoint3d(0, -10, 0);
            proj.transformPoint(bottom);
            expect(bottom.y).toBeCloseTo(-1, 5);
            // Top edge -> NDC +1
            const top = new IgePoint3d_1.IgePoint3d(0, 10, 0);
    ...
    describe("full MVP pipeline", () => {
        it("maps world point through view and projection to correct NDC", () => {
            ...
            const view = new IgeMatrix4_1.IgeMatrix4();
            view.lookAt(new IgePoint3d_1.IgePoint3d(0, 0, 10), new IgePoint3d_1.IgePoint3d(0, 0, 0), new IgePoint3d_1.IgePoint3d(0, 1, 0));
            const proj = new IgeMatrix4_1.IgeMatrix4();
            ...
            const point = new IgePoint3d_1.IgePoint3d(5, 3, 0);
        ...
        it("+X in world maps to right on screen", () => {
            ...
            const mvp = proj.clone();
            mvp.multiply(view);
            const pointLeft = new IgePoint3d_1.IgePoint3d(-5, 0, 0);
            const pointRight = new IgePoint3d_1.IgePoint3d(5, 0, 0);
        ...
        it("+Y in world maps to up on screen (higher NDC y)", () => {
            ...
            const mvp = proj.clone();
            mvp.multiply(view);
            const pointDown = new IgePoint3d_1.IgePoint3d(0, -5, 0);
            const pointUp = new IgePoint3d_1.IgePoint3d(0, 5, 0);
        ...
        it("objects further from camera have more negative Z in view space", () => {
            ...
            const near = new IgePoint3d_1.IgePoint3d(0, 0, 5);
            view.transformPoint(near);
            // Object far from camera (z=-5, camera at z=10)
            const far = new IgePoint3d_1.IgePoint3d(0, 0, -5);
    ...
    describe("rotation direction (right-hand rule)", () => {
        it("+X rotation: +Y rotates toward +Z", () => {
            ...
            const p = new IgePoint3d_1.IgePoint3d(0, 1, 0);
            m.transformVector(p);
        ...
        it("+Y rotation: +Z rotates toward +X", () => {
            ...
            const p = new IgePoint3d_1.IgePoint3d(0, 0, 1);
            m.transformVector(p);
        ...
        it("+Z rotation: +X rotates toward +Y", () => {
            ...
            const p = new IgePoint3d_1.IgePoint3d(1, 0, 0);
            m.transformVector(p);
            expect(p.x).toBeCloseTo(0, 5);
    ...
    describe("perspective projection", () => {
        it("preserves +X right, +Y up convention", () => {
            ...
            const right = new IgePoint3d_1.IgePoint3d(1, 0, -5);
            proj.transformPoint(right);
            // Point at (-1, 0, -5): to the left
            const left = new IgePoint3d_1.IgePoint3d(-1, 0, -5);
            proj.transformPoint(left);
            // Right should have higher x in NDC
            expect(right.x).toBeGreaterThan(left.x);
            // Point above should have higher y in NDC
            const up = new IgePoint3d_1.IgePoint3d(0, 1, -5);
            proj.transformPoint(up);
            const down = new IgePoint3d_1.IgePoint3d(0, -1, -5);
    ...
    describe("isometric projection formula (IgePoint3d.toIso)", () => {
        ...
        it("is consistent: X and Y both move screen-down equally", () => {
            const px = new IgePoint3d_1.IgePoint3d(1, 0, 0).toIso();
            const py = new IgePoint3d_1.IgePoint3d(0, 1, 0).toIso();
    ...
    describe("NDC to screen coordinate mapping", () => {
        ...
        it("positive world Y -> lower screen Y (higher on screen)", () => {
            ...
            mvp.multiply(view);
            // Point at world Y=+80
            const point = new IgePoint3d_1.IgePoint3d(0, 80, 0);
            mvp.transformPoint(point);
