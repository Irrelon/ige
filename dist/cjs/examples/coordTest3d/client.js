...
class Client extends IgeBaseClass_1.IgeBaseClass {
    ...
    init() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            ambient.intensity(1);
            ambient.mount(scene);
            (_a = this.renderer.lightManager) === null || _a === void 0 ? void 0 : _a.addLight(ambient);
            // Create colored cubes along each axis + origin
            const cubeSize = 20;
            const axisDist = 50;
            const cubeGeometry = IgePrimitiveGeometry_1.IgePrimitiveGeometry.createCube(cubeSize, "axis_cube");
            const cubeConfigs = [
                { name: "origin_white", pos: new IgePoint3d_1.IgePoint3d(0, 0, 0), color: { r: 1, g: 1, b: 1 } },
                { name: "+X_red", pos: new IgePoint3d_1.IgePoint3d(axisDist, 0, 0), color: { r: 1, g: 0, b: 0 } },
                { name: "+Y_green", pos: new IgePoint3d_1.IgePoint3d(0, axisDist, 0), color: { r: 0, g: 1, b: 0 } },
                { name: "+Z_blue", pos: new IgePoint3d_1.IgePoint3d(0, 0, axisDist), color: { r: 0, g: 0, b: 1 } }
            ];
            for (const cfg of cubeConfigs) {
                const entity = new IgeEntity_1.IgeEntity();
                entity.id(cfg.name);
                entity._geometryData = Object.assign(Object.assign({}, cubeGeometry), { id: `geom_${cfg.name}` });
            ...
            (0, coordTestUtils_1.displayResults)(allResults);
        });
    }
    _testView(viewName, checks) {
        ...
        const positions = {
            "origin": new IgePoint3d_1.IgePoint3d(0, 0, 0),
            "+X": new IgePoint3d_1.IgePoint3d(50, 0, 0),
            "+Y": new IgePoint3d_1.IgePoint3d(0, 50, 0),
            "+Z": new IgePoint3d_1.IgePoint3d(0, 0, 50)
        };
        // Compute screen positions
        const screenPos = {};
        for (const [key, worldPos] of Object.entries(positions)) {
            screenPos[key] = (0, coordTestUtils_1.worldToScreen)(worldPos, matrices.view, matrices.projection, this.canvasW, this.canvasH);
        ...
        for (const [key, rgb] of Object.entries(colorMap)) {
            ...
            if (sp.x >= 0 && sp.x < this.canvasW && sp.y >= 0 && sp.y < this.canvasH) {
                results.push(Object.assign(Object.assign({}, (0, coordTestUtils_1.assertPixelColor)(pixels, sp.x, sp.y, this.canvasW, this.canvasH, rgb[0], rgb[1], rgb[2], 60)), { name: `[${viewName}] ${key} color` }));
            }
        }
        // Directional checks
