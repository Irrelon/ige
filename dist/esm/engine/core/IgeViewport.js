...
export class IgeViewport extends IgeUiEntity {
     ...
     */
    screenPosition() {
        return new IgePoint3d(Math.floor(this._worldMatrix.matrix[2] + (ige.engine._bounds2d?.x2 || 0)), Math.floor(this._worldMatrix.matrix[5] + (ige.engine._bounds2d?.y2 || 0)), 0);
    }
    drawViewArea(val) {
        if (val !== undefined) {
