...
export class IgePathFinder extends IgeEventingClass {
    ...
    generate(tileMap, startPoint, endPoint, comparisonCallback = () => true, allowSquare = false, allowDiagonal = false, allowInvalidDestination = false) {
        ...
        const endPointCheckTile = tileMap.map.tileData(endPoint.x, endPoint.y) || null;
        ...
        const startNode = new IgePathNode(startPoint.x, startPoint.y, startPoint.z, 0, this._heuristic(startPoint.x, startPoint.y, startPoint.z, endPoint.x, endPoint.y, endPoint.z, 10));
    ...
    _getNeighbours(currentNode, endPoint, tileMap, comparisonCallback, allowSquare, allowDiagonal) {
        ...
        if (allowSquare) {
            ...
            if (comparisonCallback(tileData, newX, newY, newZ, currentNodeData, x, y, z)) {
                const newNode = new IgePathNode(newX, newY, newZ, currentNode.g, this._squareCost, this._heuristic(newX, newY, newZ, endPoint.x, endPoint.y, endPoint.z, this._squareCost), currentNode, "W");
                list.push(newNode);
            }
            newX = x + 1;
            newY = y;
            tileData = tileMap.map.tileData(newX, newY) || null;
            if (comparisonCallback(tileData, newX, newY, newZ, currentNodeData, x, y, z)) {
                const newNode = new IgePathNode(newX, newY, newZ, currentNode.g, this._squareCost, this._heuristic(newX, newY, newZ, endPoint.x, endPoint.y, endPoint.z, this._squareCost), currentNode, "E");
                list.push(newNode);
            }
            newX = x;
            newY = y - 1;
            tileData = tileMap.map.tileData(newX, newY) || null;
            if (comparisonCallback(tileData, newX, newY, newZ, currentNodeData, x, y, z)) {
                const newNode = new IgePathNode(newX, newY, newZ, currentNode.g, this._squareCost, this._heuristic(newX, newY, newZ, endPoint.x, endPoint.y, endPoint.z, this._squareCost), currentNode, "N");
                list.push(newNode);
            }
            newX = x;
            newY = y + 1;
            tileData = tileMap.map.tileData(newX, newY) || null;
            if (comparisonCallback(tileData, newX, newY, newZ, currentNodeData, x, y, z)) {
                const newNode = new IgePathNode(newX, newY, newZ, currentNode.g, this._squareCost, this._heuristic(newX, newY, newZ, endPoint.x, endPoint.y, endPoint.z, this._squareCost), currentNode, "S");
                list.push(newNode);
            }
        }
        if (allowDiagonal) {
            ...
            tileData = tileMap.map.tileData(newX, newY) || null;
            if (comparisonCallback(tileData, newX, newY, newZ, currentNodeData, x, y, z)) {
                const newNode = new IgePathNode(newX, newY, newZ, currentNode.g, this._diagonalCost, this._heuristic(newX, newY, newZ, endPoint.x, endPoint.y, endPoint.z, this._diagonalCost), currentNode, "NW");
                list.push(newNode);
            }
            newX = x + 1;
            newY = y - 1;
            tileData = tileMap.map.tileData(newX, newY) || null;
            if (comparisonCallback(tileData, newX, newY, newZ, currentNodeData, x, y, z)) {
                const newNode = new IgePathNode(newX, newY, newZ, currentNode.g, this._diagonalCost, this._heuristic(newX, newY, newZ, endPoint.x, endPoint.y, endPoint.z, this._diagonalCost), currentNode, "NE");
                list.push(newNode);
            }
            newX = x - 1;
            newY = y + 1;
            tileData = tileMap.map.tileData(newX, newY) || null;
            if (comparisonCallback(tileData, newX, newY, newZ, currentNodeData, x, y, z)) {
                const newNode = new IgePathNode(newX, newY, newZ, currentNode.g, this._diagonalCost, this._heuristic(newX, newY, newZ, endPoint.x, endPoint.y, endPoint.z, this._diagonalCost), currentNode, "SW");
                list.push(newNode);
            }
            newX = x + 1;
            newY = y + 1;
            tileData = tileMap.map.tileData(newX, newY) || null;
            if (comparisonCallback(tileData, newX, newY, newZ, currentNodeData, x, y, z)) {
                const newNode = new IgePathNode(newX, newY, newZ, currentNode.g, this._diagonalCost, this._heuristic(newX, newY, newZ, endPoint.x, endPoint.y, endPoint.z, this._diagonalCost), currentNode, "SE");
                list.push(newNode);
            }
        }
        return list;
     ...
     * @private
     */
    _heuristic(x1, y1, z1, x2, y2, z2, moveCost) {
