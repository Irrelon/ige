...
export class IgeFSM extends IgeBaseClass {
    ...
    enterState = (newStateName, ...rest) => {
        ...
        this._transitionQueue.push((resolve) => {
            ...
            if (!this._transitions[this._currentStateName] ||
     ...
     * @private
     */
    _transitionStates = (newStateName, ...rest) => {
        return new Promise((resolve) => {
            const currentStateObj = this.getState(this._currentStateName);
            ...
            if (!currentStateObj || !newStateObj) {
                return resolve(new Error(`Cannot change states from "${this._currentStateName}" to "${newStateName}" states because at least one is not defined.`));
            }
            if (this._debug) {
