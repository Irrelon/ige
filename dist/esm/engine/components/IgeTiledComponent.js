    ...
    _processData = (data, callback) => {
        ...
        if (isClient) {
            onLoadFunc = function (_textures, _tileSetCount, _tileSetItem) {
                return function () {
                    let cc;
                    const cs = new IgeCellSheet(_tileSetItem.name, _tileSetItem.image, this.width / _tileSetItem.tilewidth, this.height / _tileSetItem.tileheight).on("loaded", function () {
                        cc = this.cellCount();
