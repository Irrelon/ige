...
printStackTrace.implementation.prototype = {
    ...
    mode: function (e) {
        if (e["arguments"] && e.stack) {
            return (this._renderMode = "chrome");
        }
        else if (e.message && typeof window !== "undefined" && window.opera) {
            return (this._renderMode = e.stacktrace ? "opera10" : "opera");
        }
        else if (e.stack) {
            return (this._renderMode = "firefox");
        }
        return (this._renderMode = "other");
    },
    /**
    ...
    other: function (curr) {
        let ANON = "{anonymous}", fnRE = /function\s*([\w\-$]+)?\s*\(/i, stack = [], fn, args, maxStackSize = 10;
    ...
    guessAnonymousFunctions: function (stack) {
        for (let i = 0; i < stack.length; ++i) {
            ...
            const frame = stack[i], m = reStack.exec(frame);
            if (m) {
                const file = m[1], lineno = m[4], charno = m[7] || 0; //m[7] is character position in Chrome
    ...
    findFunctionName: function (source, lineNo) {
        ...
        const reFunctionExpression = /['"]?([0-9A-Za-z_]+)['"]?\s*[:=]\s*function\b/;
        // {name} = eval()
        const reFunctionEvaluation = /['"]?([0-9A-Za-z_]+)['"]?\s*[:=]\s*(?:eval|new Function)\b/;
        // Walk backwards in the source lines until we find
        // the line which matches one of the patterns above
        let code = "", line, maxLines = 10, m;
