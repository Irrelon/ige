...
import { distance } from "../../export/exports.js"
...
export class IgePathComponent extends IgeComponent {
     ...
     * destination tile.
     */
    set = (fromX, fromY, fromZ, toX, toY, toZ, findNearest = false) => {
        // Clear existing path
        this.clear();
        if (!this._finder)
     ...
     */
    add = (x, y, z, allowInvalidDestination = false) => {
        if (!this._finder)
     ...
     * character might want to be moved as close to the wall tile as is allowed.
     */
    reRoute = (x, y, z, allowInvalidDestination = false) => {
        if (!this._finder)
            throw new Error("No path finder (IgePathFinder) assigned to IgePathComponent");
        if (!this._tileMap)
            throw new Error("No tile map (IgeTileMap2d) assigned to IgePathComponent");
            ...
            if (pointFrom && pointTo) {
                if (this._currentPointFrom !== this._previousPointFrom) {
                    ...
                    while (this._nextPointToProcess < this._currentPointFrom) {
                        ...
                        this.emit("pointComplete", this._entity, new IgePoint3d(pointCurrent.x, pointCurrent.y, pointCurrent.z), new IgePoint3d(pointNext.x, pointNext.y, pointNext.z), p, p + 1, effectiveTime);
                ...
                if (this._dynamic) {
                    ...
                    if (dynamicResult === true) {
                        ...
                        this.emit("pathRecalculated", this._entity, new IgePoint3d(previousPointFrom.x, previousPointFrom.y, previousPointFrom.z), new IgePoint3d(pointFrom.x, pointFrom.y, pointFrom.z));
                    }
                    if (dynamicResult === -1) {
                ...
                const previousPointFrom = pointArr[this._previousPointFrom];
                this.emit("pathComplete", this._entity, new IgePoint3d(previousPointFrom.x, previousPointFrom.y, previousPointFrom.z), effectiveTime);
            }
        }
    ...
    _positionAlongVector = (p1, p2, speed, deltaTime) => {
        ...
        if (deltaX !== 0 || deltaY !== 0 || deltaZ !== 0) {
            return new IgePoint3d(p1X + normalisedX * (speed * deltaTime), p1Y + normalisedY * (speed * deltaTime), p1Z + normalisedZ * (speed * deltaTime));
        }
